...
const serverEnvSchema = z
  .object({
     ...
     * rotating the other.
     *
     * Required in `production` (see the refinement below). Absent elsewhere the
     * process still boots, but any credential read or write fails closed — there
     * is no plaintext fallback. Read it through `credentialVault()` in
     * `@alfred/db/credential-vault`, never directly.
     */
  ...
  .superRefine((env, ctx) => {
    // Production must not boot without the credential KEK. Every other
    // environment may omit it — a developer with no connected integrations does
    // not need one — but the vault fails closed the moment a credential is read
    // or written, so this is a hard requirement wherever real tokens live.
    if (env.NODE_ENV === "production" && !env.OAUTH_CREDENTIAL_KEK) {
      ctx.addIssue({
        code: "custom",
        path: ["OAUTH_CREDENTIAL_KEK"],
