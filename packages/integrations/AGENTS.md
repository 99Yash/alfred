...
- Build Google passthrough authority through the user-bound facade; application tools should pass `ctx.integrations.google.<service>.passthroughProfile(credentialId)` to `runRestPassthrough` with the same service slug so tokens stay integration-owned and the read gate stays coupled to its pinned API namespace.
