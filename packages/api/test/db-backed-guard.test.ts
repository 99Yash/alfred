...
// `api-tests`. This suite reads no service variable, so it registers and runs
// in every `api-tests` run — including one that reaches no database.
describe("decideDbBackedSkip", () => {
  ...
  test("fails inside CI and names the absent variable and the job", () => {
    ...
    assert.match(decision.message, /api-tests/);
  });
});
