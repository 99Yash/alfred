...
import { buildBriefingSourcePanels, referencesFromSections } from "./references";
...
const BRIEFING_COMPOSER_SYSTEM_PROMPT = [
  ...
  "Use only the provided gather payload.",
  ...
  "Use references like [[email:<documentId>]], [[meeting:<eventId>]], or [[activity:<id>]] when a section depends on a source item.",
  "Section `why` fields should explain the inclusion in one sentence, not model reasoning.",
].join("\n");
