...
- action_needed: the user must take a concrete step that isn't time-critical. Reply, decide, complete a task, rotate a credential, update a card before its actual deadline, verify identity, fix a broken build, respond to a code review. (Self-initiated sign-in/magic links, one-time login codes, and email-verification the user just requested are NOT here — they are fyi per rule 15. Persistent account-security changes such as passkeys, 2FA, OAuth apps, recovery emails, or login methods being added/changed are surfaced here unless they include a same-day compromise signal.)
...
15. Self-initiated authentication codes are the EXPECTED ECHO of an action the user just took → fyi, not action_needed and not urgent. This covers sign-in / magic links, one-time login codes AND step-up / sudo / re-authentication codes ("Sudo email verification code", "your verification code is 123456"), and email-address verification the user just requested. The user is mid-flow; a code expires harmlessly — nothing to track, nothing to remember (rule 16c). DO NOT extend this demotion to persistent account-security changes ("passkey created", "two-factor authentication enabled", "new login method added", "recovery email updated", "OAuth application added"): the email alone cannot prove the user initiated them, and these changes affect future account access. Keep them surfaced at 'action_needed' (not 'urgent' absent an explicit same-day compromise signal). Reserve urgent for the unsolicited inverse: an unrecognized sign-in, a "was this you?" challenge, account compromised, password/2FA changed without the user, or any body that says to secure the account immediately.
...
- "[GitHub] Sudo email verification code" the user just triggered → fyi (rule 15: a self-initiated step-up code, moot by the time it surfaces), no todo (rule 16c). Contrast "Passkey created" / "Two-factor authentication enabled" / "A third-party OAuth application was added to your account" → action_needed (persistent account-security changes are ambiguous without action-state, keep surfaced but not urgent), and "Sign-in from a NEW device — was this you?" → urgent (unsolicited).
