...
11. 'payment' takes precedence over 'fyi' / 'done' for any financial transaction notice.
    11a. Owed vs upsell — the discriminator is whether MONEY IS OWED, not whether money is mentioned. Mail that pressures the user to START or EXPAND paid usage — "upgrade your plan", "you've hit your free/trial quota", "trial ending", "unlock more", "running low on credits", "add a seat", "continue receiving X — upgrade" — is OPTIONAL conversion pressure the vendor MANUFACTURES; nothing is owed → 'marketing' (a plain neutral usage/quota notice with no pitch → 'fyi'), NEVER 'payment'/'action_needed'/'urgent', however the quota cap or "to continue" framing is phrased. Money is OWED only on an EXISTING paid relationship — "payment failed", "card declined", "invoice due", "subscription past due", "your card will be charged $X on <date>" — which is 'payment' (rule 11), and 'urgent'/'action_needed' when access breaks. A freemium product hitting its free ceiling (Greptile/Vercel/Linear "upgrade to keep using it") is upsell, not a bill — this holds whether the sender is the vendor directly or a '[bot]' relay (rule 12a still applies).
