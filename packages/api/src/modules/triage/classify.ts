...
15. Self-initiated authentication & security mail is the EXPECTED ECHO of an action the user just took → fyi, not action_needed and not urgent. This covers (a) sign-in / magic links, one-time login codes AND step-up / sudo / re-authentication codes ("Sudo email verification code", "your verification code is 123456"), and email-address verification the user just requested; and (b) CONFIRMATIONS of a security change the user just made — "passkey created", "two-factor authentication enabled", "security verification completed", "new login method added", "recovery email updated". The user initiated the flow and is mid-flow or just finished; a code expires harmlessly and a confirmation is moot by the time it surfaces — nothing to track, nothing to remember (rule 16c). Reserve urgent for the UNSOLICITED inverse: a sign-in or change the user did NOT initiate — an unrecognized sign-in, a "was this you?" challenge, a password/2FA change the user did not make. BOUNDARY: "a third-party OAuth application was added to your account" is NOT a code/confirmation — it can equally be the unsolicited-compromise signal and the email alone cannot tell, so treat it as the unsolicited class and keep it surfaced at 'action_needed' (not 'urgent' absent a same-day breach signal, not 'fyi').
...
- "[GitHub] Sudo email verification code" / "Passkey created" / "Two-factor authentication enabled" the user just triggered → fyi (rule 15: a self-initiated step-up code, or the confirmation echo of a security change the user just made — moot by the time it surfaces), no todo (rule 16c). Contrast "Sign-in from a NEW device — was this you?" → urgent (unsolicited), and "A third-party OAuth application was added to your account" → action_needed (rule 15 boundary: possibly unsolicited, keep surfaced but not urgent).
...
export function detectConflict(
  ...
  if (
    ...
    const isService =
      observations.senderKind?.kind === "service" || (priorKey?.startsWith("service:") ?? false);
