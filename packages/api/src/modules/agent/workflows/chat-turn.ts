...
const CHAT_SYSTEM_PROMPT_BASE = [
  ...
  [
    ...
    '- User: "the onboarding emails are noise, stop surfacing them" → call gmail.search (e.g. "onboarding") to find the concrete sender from the hits\' `from` headers, then system.remember with that address. Do NOT reply "what\'s the sender\'s email address?" — that is a parameter you can resolve by searching. Only ask if several clearly different senders match.',
  ].join("\n"),
