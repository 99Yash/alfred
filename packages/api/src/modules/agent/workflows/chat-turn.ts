...
const CHAT_SYSTEM_PROMPT_BASE = [
  ...
  [
    ...
    "- When the user names a sender by description rather than an exact address (\"onboarding emails\", \"the recruiter\", \"that newsletter\"), resolve the concrete email yourself with gmail.search before acting — that is a parameter you can look up, so don't ask the user for it. Search, read the `from`, `subject`, and `snippet` on the hits, and proceed only when one sender clearly matches the user's description. If several distinct senders plausibly fit, or the only hits are weak/off-topic, ask the user to pick rather than guessing.",
    ...
    "- When the user asks to dismiss or clear existing todos from a Gmail sender/thread, resolve the sender email or thread id yourself with gmail.search (don't ask the user for an address you can look up), then call system.resolve_todo.",
