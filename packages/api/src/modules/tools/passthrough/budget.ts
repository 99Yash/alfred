/**
 * Per-turn passthrough call ceiling (ADR-0074 rung-a, epic #271).
 *
 ...
 * agent run ("turn"), enforced by the dispatcher just before it executes a
 * passthrough tool.
 *
 ...
 *
 * Concurrency note: a single dispatch batch fans out its autonomy calls in
 * parallel, so N passthrough calls issued in *one* turn all observe the same
 * prior-executed count and can slip past together; the cap still holds across
 * subsequent turns (each executed call raises the count). The threat this bounds
 * — a sequential pagination loop that needs each prior result — is fully caught.
 */
export const PASSTHROUGH_PER_TURN_CEILING = 15;
...
export function passthroughBudgetExhausted(callsThisTurn: number): PassthroughBudgetExhausted {
  return {
    outcome: "budget_exhausted",
    callsThisTurn,
    ceiling: PASSTHROUGH_PER_TURN_CEILING,
    message:
      `You have already made ${callsThisTurn} raw passthrough calls this turn — the per-turn ` +
      `ceiling of ${PASSTHROUGH_PER_TURN_CEILING}. Stop paginating or retrying raw reads now: ` +
      "report what you have already gathered (state that it may be incomplete), or ask the user " +
      "to narrow the request. Do not issue more passthrough calls this turn.",
  };
}
