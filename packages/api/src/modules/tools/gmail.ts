...
const gmailSearchInput = z
	.object({
		...
		maxResults: z
			...
			.default(10)
			.describe(
				"Cap on results returned to the model (Gmail allows up to 500; we cap at 50).",
			),
	})
	.strict();
...
const gmailReadMessageInput = z
	.object({
		documentId: z
			...
			.optional()
			.describe(
				"Alfred document id for an ingested Gmail message. Prefer this when available.",
			),
		messageId: z
			.string()
			.min(1)
			.optional()
			.describe(
				"Provider-native Gmail message id. Use only when no Alfred document id is known.",
			),
	})
	.strict()
...
export const gmailTools: readonly RegisteredTool[] = [
	liveTool({
		...
		riskTier: "no_risk",
		description:
			"Search Gmail messages using Gmail query operators and return message ids.",
	...
	liveTool({
		...
		riskTier: "low",
		description:
			"Read the cached full text and metadata for one ingested Gmail message.",
	...
	liveTool({
		...
		riskTier: "high",
		description:
			"Prepare or send a Gmail draft after the user approves the proposed message.",
