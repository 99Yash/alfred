...
- Build HTTP failures with `httpErrorFromResponse` and test them with `isHttpError`. Bound and redact anything derived from a response body with `summarizeBody` / `redactSecrets` before it reaches a log, and `sanitizeErrorMessage` before it reaches a model. A raw upstream body riding an error is how credentials leak.
