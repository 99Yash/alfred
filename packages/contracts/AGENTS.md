...
- `extractEmailAddress` (loose) and `normalizeEmailAddress` (strict) in `domain.ts` are the address rules — `parseEmailAddress` delegates to the loose one and stays the single source of self-mail matching. Do not hand-parse `Name <addr>` headers or lowercase addresses inline.
...
- Read every per-integration fact off the registry entry `INTEGRATIONS[slug]` in `src/integrations/registry.ts` (ADR-0093). The record's keys are the slug space; do not spell a slug list anywhere else. Derive a slug-keyed table from the record or key it `satisfies Record<…Slug, T>` on a derived union; do not hand-type a `Partial<Record<IntegrationSlug, …>>`.
