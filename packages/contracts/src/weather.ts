...
/**
 * Keys are IANA timezone strings (cast-narrowed to `IanaTimezone` at the
 * lookup site via `isIanaTimezone`). The Record uses the branded alias so
 * callers can index by a value typed as `IanaTimezone` without a manual cast.
 */
export const WEATHER_FALLBACK_CITIES: Record<IanaTimezone, WeatherFallbackLocation> = {
...
} as unknown as Record<IanaTimezone, WeatherFallbackLocation>;
...
export function weatherFallbackFor(tz: IanaTimezone): WeatherFallbackLocation | null {
  return WEATHER_FALLBACK_CITIES[tz] ?? null;
}
