...
- "Additional information needed for the position 26-1620" from a recruiter, with \`spam=true\` → action_needed (rule 20's EXCEPTION: the user opened this application themselves, so the ask is an obligation the user already owns and it does not depend on trusting the sender — Gmail's filter is wrong here). Contrast "URGENT: verify your billing details within 24 hours" with \`spam=true\` → fyi (the demand works only if the sender is honest — phish).
