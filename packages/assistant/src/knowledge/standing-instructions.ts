 ...
 *   3. the insert path, where the new row is not in `locked` — the snapshot
 *      the report is computed from — and so reaches neither. Absence from
 *      `locked` proves only that much. It does not identify the new row: a row
 *      a concurrent transaction inserted for the SAME target can be absent
 *      from `locked` too (campaign item 46). The claim this path needs is the
 *      weaker one — a row outside the reported snapshot cannot appear in the
 *      report — so the new row never names itself.
 *
