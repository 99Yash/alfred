...
describe("event-dispatch duplicate-run guard (#531)", { skip: SKIP }, () => {
  ...
  test("concurrent trigger publications create exactly one workflow run", async () => {
    ...
    // `gmail.documents_ingested` batch consumers, and the `github-activity-fold`
    // consumer, which all no-op on this `message_received` event but still
    // accept it. The duplicate-run guard is the run count below, not the
    // consumer count.
