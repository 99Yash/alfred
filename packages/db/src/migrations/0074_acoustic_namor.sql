...
SET "source" = '{"kind":"agent"}'::jsonb
WHERE "source" = '{}'::jsonb;--> statement-breakpoint
UPDATE "memory_chunks"
SET "source" = jsonb_build_object(
  'kind', 'agent',
  'meta', jsonb_build_object('previousSource', "source")
)
WHERE jsonb_typeof("source") <> 'object'
  OR "source" ->> 'kind' IS NULL
  OR "source" ->> 'kind' NOT IN ('document', 'chunk', 'tool_call', 'cold_start', 'user', 'agent');--> statement-breakpoint
UPDATE "user_facts"
SET "source" = '{"kind":"agent"}'::jsonb
WHERE "source" = '{}'::jsonb;--> statement-breakpoint
UPDATE "user_facts"
