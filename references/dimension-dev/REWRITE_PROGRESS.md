...
- **2026-05-20 — Integration detail hero + trust dial + Connected layout** — Closed the three biggest visual gaps on `/integrations/$provider` against the Dimension reference. (1) Added a 200px-tall `HeroPreview` showcase between the header and Connected section: dark radial-gradient surface + subtle grid overlay + three floating tiles (`88px / 120px / 88px`) repeating the provider glyph; same shape for connected, available, and coming-soon variants. (2) Restructured the Connected section away from a bordered table-grid to Dimension's "three uppercase column labels above a flat account row" layout (Connected / Date / Status, then `Connected account` + `Mar 17, 2026` + emerald-dot Active + destructive Disconnect pill); unconnected state renders a single muted "No account connected yet." line. (3) Added a hand-rolled SVG dial illustration (`TrustDial`) anchored to the right edge of the trust card — 36 ticks with every 9th elongated, two concentric rings, a center dot, and a soft purple radial glow — matching the Dimension "Your data is indexed & encrypted" card. Verified live on `/integrations/google_gmail`, `/integrations/slack` (empty-connected variant), and `/integrations/google_drive` (related-provider variant — "Complete your Google Setup" rows still rendered correctly below the trust card). Type-check clean (`pnpm --filter web check-types`).
- **2026-05-20 — Settings panels + quick rail sub-states** — Closed queue slices #9 and #10. (1) `/settings`: Features panel rebuilt around a "Background Agents" cluster with six toggles (Action Items, Evening Recap, Morning Briefing, Email Tagging, Email Auto-Drafting, Meeting Prep) matching Dimension 08b — each row has a 28px frost-tile leading icon, label, helper, and `Switch`. Preferences rebuilt into three sub-clusters (`Appearance` / `Model` / `Email Preferences`) matching the multi-cluster Dimension grammar in 08c — Theme tabs now carry sun/moon/sliders icons; Product Updates ride on a switch. Added a generic `Cluster` helper (13px medium title + 5-gap body) and an `AgentRow` helper so future feature catalogs reuse the same shape. Inner-nav rows bumped to `h-8` with the purple active bar pulled to `-left-2` so it sits in the gutter; on mobile the inner-nav now stays **vertical** (matching Dimension 19d) instead of becoming a horizontal scroll strip — the wrapper grid is `grid-cols-1` on mobile and `md:grid-cols-[200px_1fr]` from `md` up. Heading scales down to `34/40` on mobile. (2) `quick-access-rail.tsx`: Emails and Meetings tabs now have populated fixture states — Emails shows three drafts (Anna Chen / Stripe / GitHub) under an `All · N drafts` header; Meetings shows three meetings under a `Today · N scheduled` header, each row carrying a frost-icon tile (Video/MapPin based on location), title + tabular time, and attendee count + location/duration meta. Empty fallbacks for both still render the Dimension "all done" state but now with a `CheckCircle2` glyph instead of the off-brand `PartyPopper`. Suggestions on the To Do tab are now interactive — clicking a suggestion moves it into the active to-do list and removes it from the suggestions cluster (reproduces Dimension's accepted-suggestion flow from screenshots 42/43). Todos themselves are clickable to toggle done/strikethrough; the inline "Add new to do" input accepts Enter to push a new row. Baselines: `settings-{user,features,preferences,mobile}-v2.png`, `rail-{tasks,emails,meetings,tasks-suggestion-accepted}-v2.png`. Type-check clean.
