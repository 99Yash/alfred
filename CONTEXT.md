...
**Event source + type.** Closed const-narrowed pair in `@alfred/contracts` (`EVENT_SOURCES`, per-source `*_EVENT_TYPES`), replacing the old informal `source: 'gmail.ingest'` string. v1: `source='gmail'`, `type='message_received'`. The `webhook|ingest|manual` breadcrumb rides `trigger.payload.reason`, not the type. `trigger.filter` stays on the schema for forward-compat but the v1 dispatcher does **not** evaluate it (API rejects non-empty `filter` writes, like `steps`); the brief-only boss decides relevance itself. ADR-0047.
