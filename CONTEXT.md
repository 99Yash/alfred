...
**Call role.** `CallRole` union (`packages/ai/src/metering/types.ts`): `'compactor' | 'boss' | 'sub_agent' | 'triage' | 'briefing' | 'cold_start' | 'memory_extraction'`. Set on `attribution.role` when calling `meteredGenerateText` / `meteredGenerateObject`; surfaces on `api_call_log.request_meta.role` so a single run's spend can be split between agent surfaces (boss vs sub-agent vs compactor) without adding a column. Phase 7 wires only `'compactor'`; the rest are backfilled in a follow-up. Query with `request_meta->>'role'` in `agent_run_context` cost rollups.
