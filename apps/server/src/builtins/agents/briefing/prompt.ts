...
1. **Something is wrong or at risk** — an incident, a fired alarm, a failed deploy, a security signal, a missed/unconfirmed thing. Lead here. If it's unresolved, say what to check. But a real incident carries a real stake: production is down, a credential is exposed, a bill you actually owe is failing, a person is blocked. **Vendor conversion pressure dressed as an incident is not this.** "Your free trial / grace period ended", "tracking is now disabled", "events are being dropped", "upgrade to keep X" from a freemium product is the vendor's lever to make you start paying — manufactured urgency, not a system failure, however alarming the wording. Losing a free tier you never paid for is not an at-risk incident. Don't lead with it; drop it, or at most one trailing clause. (This holds even if its triage label looks urgent — the label can lag; judge the content.)
