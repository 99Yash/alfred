...
import { createSmokeRun, findOrCreateSmokeUser } from "./_smoke-helpers";
...
async function main(): Promise<void> {
  ...
  const userId = await findOrCreateSmokeUser("smoke-scratchpad@alfred.local");
  const runId = await createSmokeRun(userId);
  console.log(`[smoke-scratchpad] user=${userId} run=${runId}`, "smoke-test");
  ...
  if (
    !subRead ||
    subRead.value.topic !== "inbox-debt" ||
    subRead.value.count !== 42
  ) {
  ...
  const sharedRead = await readScratch<string>({
    runId,
    zone: "shared",
    path: "goal",
  });
  ...
  if (
    !promoted ||
    !deepEqual(promoted.value, { topic: "inbox-debt", count: 42 })
  ) {
  ...
  console.log(
    "[smoke-scratchpad] promote ok (scratch.subA.findings → shared.findings)",
  );
  ...
  if (
    dispatchShared?.value.ok !== true ||
    dispatchShared.writtenBy !== "boss"
  ) {
  ...
  if (subWrite.kind !== "executed") {
    throw new Error(
      `[smoke-scratchpad] sub own write expected executed, got ${subWrite.kind}`,
    );
  }
  ...
  if (dispatchSub?.value !== "sub-owned" || dispatchSub.writtenBy !== "subA") {
    throw new Error(
      "[smoke-scratchpad] sub-agent own scratch write did not land",
    );
  }
  ...
  const promotedByDispatch = await dispatchToolCall({
    ...
    input: {
      fromKey: "scratch.subA.dispatch",
      toKey: "shared.dispatch_promoted",
    },
    userId,
    caller: "boss",
  });
  ...
  if (dispatchPromoted?.value !== "sub-owned") {
    throw new Error(
      "[smoke-scratchpad] dispatcher promote did not copy the sub-agent value",
    );
  }
  const scratchToolRows = await db()
    ...
    .where(
      and(
        ...
        eq(actionStagings.stepId, "scratch-tools"),
      ),
    );
  ...
  console.log(
    "[smoke-scratchpad] dispatcher scratch tools + zone enforcement ok",
  );
  ...
  if (firstCount !== 6) {
    throw new Error(
      `[smoke-scratchpad] expected 6 snapshot rows, got ${firstCount}`,
    );
  }
  ...
  console.log(
    `[smoke-scratchpad] second snapshot idempotent (${secondCount} rows, same shape)`,
  );
  ...
  try {
    ...
    let cursor = "0";
    do {
      const [next, batch] = await conn.scan(
        cursor,
        "MATCH",
        match,
        "COUNT",
