 ...
 */
const KNOWN_PRESENTATION: Readonly<
	Record<string, { icon: LucideIcon; tint: WorkflowTint }>
> = {
...
const FALLBACK_TINTS: ReadonlyArray<WorkflowTint> = [
	"violet",
	"emerald",
	"amber",
];
...
function presentationFor(w: SyncedWorkflow): {
	...
	const tint =
		FALLBACK_TINTS[hashSlug(w.slug) % FALLBACK_TINTS.length] ?? "violet";
	const icon =
		w.trigger.kind === "cron"
			? CalendarClock
...
export function syncedWorkflowToView(w: SyncedWorkflow): WorkflowDefinition {
	...
	return {
		...
		integrations:
			w.allowedIntegrations.length > 0
				? w.allowedIntegrations.map(titleCase)
				: ["—"],
	};
}
