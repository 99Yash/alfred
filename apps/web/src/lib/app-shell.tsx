...
export function AppShell({ children }: { children: ReactNode }) {
   ...
   * threads) until Replicache wires real data in m13; the preview routes
   * pass demo fixtures so the design surface stays loud. */
  const isPreviewRoute = location.pathname.startsWith("/preview/");
  const sidebarThreads = isPreviewRoute ? PREVIEW_CHAT_THREADS : undefined;
