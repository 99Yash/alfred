/**
 * Fixture data for /skills + /skills/$slug.
 *
...
export type SkillFixtureTint = "violet" | "sky" | "amber" | "emerald";

export interface SkillFixtureRun {
  id: string;
  kind: "learn" | "re-learn";
...
export interface SkillFixture {
  ...
  description: string;
  status: "active" | "draft";
  tint: SkillFixtureTint;
  prompt: string;
  memoryBody: string;
  lastRunAt: string;
  updatedAt: string;
  integrations: ReadonlyArray<string>;
  runs: ReadonlyArray<SkillFixtureRun>;
}

const daysAgo = (days: number) => new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();
const hoursAgo = (h: number) => new Date(Date.now() - h * 60 * 60 * 1000).toISOString();

export const SKILL_FIXTURES: ReadonlyArray<SkillFixture> = [
  {
...
export function findSkillFixture(slug: string): SkillFixture | undefined {
  return SKILL_FIXTURES.find((s) => s.slug === slug);
}
