					...
					{builtins.length > 0 ? (
						<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
							{builtins.map((w, i) => (
								<WorkflowCard
									key={w.slug}
									workflow={syncedWorkflowToView(w)}
									index={i}
								/>
							))}
						</div>
						...
						<p className="px-1 text-xs text-vs-fg-3">
							{loading
								? "Loading workflows…"
								: "No built-in workflows seeded yet."}
						</p>
					)}
				</section>
					...
					{authored.length > 0 ? (
						...
						>
							...
							<p className="max-w-[28rem] text-xs text-vs-fg-3 leading-5">
								Author your own scheduled or event-driven flows from chat, then
								tune their trigger and integrations here.
							</p>
						</VsCard>
					)}
				</section>
