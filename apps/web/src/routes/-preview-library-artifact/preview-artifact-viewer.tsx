    ...
    return (
      ...
      >
        <button
          ...
          aria-label="Close"
          onClick={close}
          className="absolute inset-0 bg-app-background/70 backdrop-blur-[2px]"
        />
        <div
          className={cn(
            "relative w-[min(420px,92vw)] rounded-2xl bg-app-bg-1 p-6 text-center",
            "shadow-[0_24px_64px_rgba(0,0,0,0.20),0_0_0_1px_rgba(0,0,0,0.06)]",
          )}
        >
          <p className="text-sm font-medium text-app-fg-4">Artifact not found</p>
          <Link
            to="/library"
            className="mt-3 inline-flex text-xs text-app-fg-3 underline underline-offset-4 hover:text-app-fg-4"
          >
            Back to Library
          </Link>
        </div>
          ...
          {artifact.pages.map((page, index) => (
            <section key={`${page.title}-${index}`} aria-label={`Page ${index + 1}`}>
              ...
              {page.html ? (
                ...
                <div
                  className={cn(
                    "min-h-[590px] rounded-2xl bg-app-bg-1 p-10 text-app-fg-4",
