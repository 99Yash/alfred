...
export function ShareDialog({ workflow, open, onClose }: ShareDialogProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative w-[min(520px,92vw)] rounded-2xl bg-app-bg-1",
          "shadow-[0_24px_64px_rgba(0,0,0,0.18),0_0_0_1px_rgba(0,0,0,0.06)]",
        )}
      >
        <div className="px-6 pt-5 pb-2">
          <p className="text-sm font-medium text-app-fg-4">Share workflow</p>
          <p className="mt-1 text-xs text-app-fg-3">Copy a private link to this workflow preview.</p>
        </div>
          ...
          <div className="rounded-2xl bg-app-bg-2/60 shadow-[0_0_0_1px_rgba(0,0,0,0.05)] p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-app-fg-4">{workflow.name}</p>
                <p className="truncate text-[12px] text-app-fg-3">{workflow.description}</p>
              </div>
            </div>
            <div
              className={cn(
                "mt-4 flex items-center gap-2 rounded-xl bg-app-bg-1 px-3 py-2",
                "shadow-[0_0_0_1px_rgba(0,0,0,0.05)]",
              )}
            >
              <Link2 size={14} className="shrink-0 text-app-fg-2" />
              <p className="min-w-0 flex-1 truncate text-[12.5px] text-app-fg-3">
                alfred.local/workflows/{workflow.id}
              </p>
              <AppButton variant="ghost" size="sm" leading={COPY_LEADING}>
                Copy
              </AppButton>
            </div>
          </div>
          <div className="mt-4 flex justify-end">
            <AppButton variant="white" size="md" onClick={onClose}>
              Close
            </AppButton>
          </div>
        </div>
