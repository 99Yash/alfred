import {
	type RiskTierCounts,
	useIntegrationTierCounts,
} from "~/hooks/use-tool-tiers";
import {
...
export function Capabilities({ provider }: { provider: IntegrationProvider }) {
	const tierCounts = useIntegrationTierCounts(
		integrationSlugForProvider(provider.id),
	);
	...
	return (
		...
		>
			<div className="flex items-baseline justify-between gap-3">
				...
				{tierCounts ? (
					<span className="text-xs text-vs-fg-3 tabular-nums">
						{summarizeTiers(tierCounts)}
					</span>
				) : null}
			</div>
