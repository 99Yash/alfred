...
- **Reliability lease cluster #136/#137/#151** — ADR-0070 edits the same lease/executor machinery: **#137** (no lease tests; "stand up a DB/tx harness") is a **prerequisite** for ADR-0070's backstop tests; **#136** (transient retry) is the **complementary half** of step-retry policy (must coexist with the give-up backstop); **#151** (write idempotency) is eased by terminal-fail. Cross-referenced here, not folded into this batch.
...
> - **The wedge is at the *error-write*, not only the `execute_result` write.** The export step sat **86.8s in `running`** then got *reclaimed* — it was never `commitStepFailure`'d. If only :694 were poisoned, the throw would reach `runOnce`'s catch → `commitStepFailure` → terminal `failed` in ~2s (no loop). Instead `finalizeFailedMessage` succeeded (the `failed/attachment` message row exists), then `commitStepFailure` re-threw persisting `agent_runs.error.message = <the poison pg error>` as jsonb → escaped `runOnce` → run stayed `running` → reclaim → re-run poison → repeat. **The failure-recording path is itself a poison sink.**

1. `@alfred/contracts`: `sanitizeToolResult(value) → { value, removed }` — recursive; strip `U+0000` + lone surrogates from strings. Unit tests (null-byte, surrogate, nested, non-string passthrough).
2. `dispatch/index.ts`: apply in **both** `executeAndCommit` (immediately after `tool.execute` returns — line ~668, so the `execute_result` write at ~694 *and* the returned `toolResult` are both clean) and `executeFastPath` (~713); on `removed > 0` set result `sanitized:true` and `console.warn` (tool + count). This also protects the transcript/state sinks (same value flows there).
3. **Sanitize the error-persistence sites too** (this is the actual wedge — promoted from ADR-0070 rejected-alt (c) to a required item). Apply the same strip to every poison-reachable error/text write:
   ...
   - `dispatch/index.ts` the `if (error)` branch (`:673-685`): sanitize the `executeError` jsonb write — reachable when a tool *throws* a null-byte message (lesser bug than #267 since sanitized `commitStepFailure` downstream stops the loop, but the same poison class; named so the sink enumeration is complete).
   - **Why required, not belt-and-suspenders:** with this, the poison export fails **fast + clean** (`commitStepFailure` succeeds → run terminal `failed` in ~2s) and **no reclaim loop ever forms** — so the backstop below is freed to guard only the residual "step genuinely can't progress" class (real hangs / event-loop blocks), exactly as intended. Without it, the backstop is the *only* guard and it resolves slow + opaque (3 reclaims ≈ 3+ min of "running").
4. `agent/executor.ts` `leaseRun` (the residual-class backstop): before re-leasing a stale-`running` row, count contiguous `agent_steps` rows for `(run_id, current_step)` with `status='failed'` and `error->>'message' LIKE 'lease reclaimed%'` since the last `completed` step; `>= 3` → terminal-fail. The terminal write **must** use the synthetic clean message `"step <id> not progressing: reclaimed N times"` and **must not** echo the original error (else the terminal write re-throws on the same poison and the loop survives the backstop). Tests: trips at 3; one genuine worker death still recovers.
...
1. **Presence gate (load-bearing).** Thread `turnHadAttachment` (from `loadReadyAttachments`, for the current user turn) into the failure path; `classifyChatFailure(err, { turnHadAttachment })` can only return `attachment` when the flag is set. A no-attachment tool/persistence fault → `generic`. **This alone resolves the reported #269.**
2. **Narrow the signal.** Keep only the genuine provider-image-reject signals (`"unable to process input image"`, the `invalid`/`unsupported image` family) and **delete the over-broad `mentionsAttachment` net** (`attachment|file|image|media|mime` + the `could not process` clause, chat-turn.ts:1319-1340) — that net is what mis-bucketed the export failure. Do **not** relocate a throw to hydration: hydration already swallows a bad image to a placeholder and continues (chat-turn.ts:444-463); leave it. *(Optional: wrap the recognized provider reject in a tagged error at the catch site for cleanliness — recognition stays signal-based, since a provider error carries no structured tag.)*
3. `finalizeFailedMessage`: mark the message `failed` only on a terminal fault (not a to-be-reclaimed error), so `chat_messages.status` and `agent_runs.status` agree. Tests: a tool failure with no attachment → `generic`, not `attachment`; a no-attachment turn never yields `attachment` regardless of the message text; message/run statuses consistent.
...
- **General tier / Code Mode / BYO-MCP** (ADR-0074, *no tracking issue yet — file one when picked up*) — needs a sandbox + cross-integration risk grill; Railway + R2 + Vercel-AI-SDK constraints (no CF Workers isolate in hand).
- **Produce-artifact epic** (ADR-0075, *no tracking issue yet — file one when picked up*) — inline sidebar + renderers, borrow from `-dimension-ai-web`.
