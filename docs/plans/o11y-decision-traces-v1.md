...
- **The normal trace seam is a step-level side effect**, emitted via a new
  `ctx.trace(kind, record)` parallel to the existing `ctx.stageAction` / `ctx.log`,
  persisted inside the step's commit transaction. A domain store may additionally
  write the same keyed trace inside its own transaction when the domain row and its
  forensic trace must commit together; the later executor insert is idempotent.
  Triage uses this exception for the canonical `email_triage` row. Workflows opt in
  per decision; triage is the first producer, the seam is generic so briefing /
  memory-extraction / cold-start adopt incrementally.
...
- `executor.ts` — collect into a `traces: TraceRecord[]` array (parallel to `staged`),
  ...
  its classification trace inside the `upsertTriage` row transaction, deriving
  `user_id`/`workflow_slug` from `agent_runs` and checking the current step/attempt, so a
  crash after the canonical tag write cannot leave the row without its forensic trace.

### Producer: triage (v1)

- `email-triage.ts:554` — replace the `JSON.stringify(senderExtractionEvent(...))`
  progress-log with a `decisionTrace` passed to `upsertTriage(...)` and the same
  `ctx.trace("triage.classification", ...)` slot for the executor commit. Keep the terse
  `classify: doc=… category=…` log for live progress.
...
1. **Schema + seam** — `agent_decision_traces` table + migration (`db:generate` →
   `db:migrate`, never push), `ctx.trace`, trace-kind registry, and the
   API-owned `SenderExtractionEvent` payload type.
...
5. **ADR + docs** — write the ADR, update memory.
