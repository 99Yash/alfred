...
**Status:** IMPLEMENTED (code + unit/write-path tests green, `pnpm check-types` clean). The **live re-probe (Verification §3) is the acceptance gate and is still pending** — do not treat this as done until a document sized to time out is proven to chunk proactively.
