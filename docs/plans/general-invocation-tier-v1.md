...
**Exhaustive coverage, not a drifting hand-written scope list.** The canonical `LOADABLE_INTEGRATION_SLUGS` remains the source of truth. A shared general-invocation coverage registry must satisfy `Record<LoadableIntegrationSlug, CoverageDecision>`, where every entry is explicitly `supported`, `deferred`, or `not_applicable`. Adding a new integration slug without classifying it is a compile error. A second API-side handler registry is keyed by the type-level subset marked `supported`; changing a coverage decision to `supported` is a compile error until its preference key, read gate, transport, tool action, and discovery description are wired. The Settings UI renders supported entries from the same registry, so there is no third provider list. v1 classifications:
