...
| `context-search`  | Cross-integration read boundary: registered read-only evidence sources, the query/task envelope, and bounded source-attributed results (epic #422) | `searchContext`, `registerContextSource`, `registerDefaultContextSources`, `listContextSources`, `packEvidenceCards`, `rankEvidenceCards`, `entitySignificanceKey`, `EVIDENCE_RANK_FEATURES`, `ContextSearchRequest`, `ContextSource`, `EvidenceRankContext`, `EvidenceRankFeature`, `EvidenceRanking`, `RankedEvidence`; adapters, the source registry internals, the user-model ranking signal, and the decay helper stay private                                                                                                                                                                                                                                 | new (#422)                                                                                |
