...
| `context-search`  | Cross-integration read boundary: registered read-only evidence sources, the query/task envelope, and bounded source-attributed results (epic #422) | `searchContext`, `registerContextSource`, `registerDefaultContextSources`, `selectContextSources`, `contextSourcePriorities`, `isTrustedRetrievalSource`, `packEvidenceCards`, `ContextSearchRequest`, `ContextSource`, `SourceExclusionReason`, `ContextSourceReport`, `EvidenceRanking`; adapters, the source registry enumeration, the deterministic ranker, the manifest priority fold, the user-model ranking signal, and the decay helper stay private (the ranker is drivable through a `test-support` door for tests and the retrieval eval)                                                                                                                                                                                                                                 | new (#422)                                                                                |
