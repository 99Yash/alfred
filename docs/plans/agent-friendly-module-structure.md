...
| `realtime` | User-event fan-out, outbox relay and retention, Replicache poke bridge, outbox replay reads | `initEventBridge`, `closeEventBridge`, `initReplicachePokeBridge`, `closeReplicachePokeBridge`, `emitReplicachePokes`, `subscribeUserEvents`, `subscribeUserPokes`, `getEventsSince`, `getReplayHighWatermark`; the relay, the reaper and the periodic-task primitive stay private | `src/realtime`; framing stays in `packages/http/src/realtime/` |
