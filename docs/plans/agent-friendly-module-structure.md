...
| `context-search`  | Cross-integration read boundary: registered read-only evidence sources, the query/task envelope, and bounded source-attributed results (epic #422) | `searchContext`, `registerContextSource`, `registerDefaultContextSources`, `listContextSources`, `selectContextSources`, `contextSourcePriorities`, `listSourceManifests`, `packEvidenceCards`, `ContextSearchRequest`, `ContextSource`, `ContextSourceSelection`, `ContextSourceExclusion`, `EvidenceRanking`; adapters, the source registry internals, the deterministic ranker, the manifest priority fold, the user-model ranking signal, and the decay helper stay private (the ranker is drivable through a `test-support` door for tests and the retrieval eval)                                                                                                                                                                                                                                 | new (#422)                                                                                |
...
| `reply-drafting`  | Reply-worthiness gate, structural verifier, Gmail send-access check, `reply-drafting` recipe, post-triage trigger consumer, `reply_drafting.decision` trace                     | `decideReplyWorthiness`, `prepareReplyStaging`, `checkGmailSendAccess`, `replyDraftingTriggerConsumer`; recipe registered by the composition root                                                                                                                                                                                                                                                                              | new (#243); depends on `triage`, never imported by it                                      |
...
flowchart TD
  ...
  triage --> tasks
  tasks --> knowledge
  reply_drafting["reply-drafting"] --> triage
  reply_drafting --> knowledge
