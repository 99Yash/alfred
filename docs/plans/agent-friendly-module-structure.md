...
| `context-search`  | Cross-integration read boundary: registered read-only evidence sources, the query/task envelope, and bounded source-attributed results (epic #422) | `searchContext`, `registerContextSource`, `registerDefaultContextSources`, `listContextSources`, `selectContextSources`, `contextSourcePriorities`, `packEvidenceCards`, `ContextSearchRequest`, `ContextSource`, `ContextSourceSelection`, `ContextSourceExclusion`, `EvidenceRanking`; adapters, the source registry internals, the deterministic ranker, the manifest priority fold, the user-model ranking signal, and the decay helper stay private (the ranker is drivable through a `test-support` door for tests and the retrieval eval)                                                                                                                                                                                                                                 | new (#422)                                                                                |
