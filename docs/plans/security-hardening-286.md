...
- **`proposedInput` write: redact only when `!requiresApproval`** (it doubles as the
  approval-resume payload at dispatch `:470` — redacting a _gated_ tool's proposedInput would
...
- **#293:** blocked signed/token URLs, allowed ordinary + `utm_*` URLs, redirect target that
  _adds_ a credential param; assert redaction in result/error/redirects + that `proposedInput`
