...
*Amended by slice #1019.* The tool is now kernel, so the chat boss sees it on every turn without a search and load. The tool description carries the when-to-ask rubric: ask only when a wrong guess wastes work, ask every question in one call, two to four options, the recommended option first with "(Recommended)" in its label, no "Other" option, and no question for the user in a sub-agent brief. The option cap moved from six to four in `ASK_USER_LIMITS`, so a question row staged before the change with five or six options fails the resume re-parse. The chat prompt gains one sentence that points at the tool. The kernel costs about 1.8 KB more on every chat prompt; `schema-budget.test.ts` records the bump.
