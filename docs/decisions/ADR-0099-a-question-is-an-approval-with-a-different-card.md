...
**Amendment (2026-09-23, #1019) — `system.ask_user` is kernel; options are capped at four.** Slice #1019 adds the tool description and the prompt guidance that **What the user sees** and **Residual risk** above defer. Three things change:

- **Option count.** Each question now has two to four options, not two to six. `ASK_USER_LIMITS` in `@alfred/contracts` sets the cap, and the schema, the field description, and the tool prose read it from there. A question row staged before the change with five or six options fails the resume re-parse against the four-option schema.
- **Kernel membership.** The tool is now kernel, so the chat boss sees it on every turn without a search and load. The chat prompt names the tool, and a lazy tool would cost a `search_tools`/`load_tool` step and a mid-turn prompt-cache bust on each turn where the model decides to ask. That cost is the reason for the choice. The "prompt-named system tools are kernel" gate in `tool-surface.test.ts` does not force it, because the gate also accepts an `intentionallyLazy` entry. `callers: ["boss"]` and `requiresLiveChat` still keep the tool off sub-agent and background surfaces. The kernel costs about 2 KB more on every live chat prompt (2,051 B measured: 9,817 → 11,868 B), and `schema-budget.test.ts` records the bump.
- **Guidance.** The tool description carries the when-to-ask rubric: ask only when a wrong guess wastes work, ask every question in one call, two to four options, the recommended option first with "(Recommended)" in its label, no "Other" option, and no question for the user in a sub-agent brief. The chat prompt gains one sentence that points at the tool.

The residual risk "the tool is lazy and has no prompt guidance" is retired. A new residual risk replaces it: no recorded ambiguous turn has yet shown that the model calls the tool instead of ending its reply with questions.
