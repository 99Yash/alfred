...
Build sequence and file-level detail live in [docs/plans/meeting-prep-v1.md](../plans/meeting-prep-v1.md); glossary terms (_Meeting prep packet_, _`meeting_preps`_, _Gated recompute_, _Prep horizon + sweep_, _Calendar watch_, _`system.prepare_meeting`_, _Prep reference_) in [CONTEXT.md](../../CONTEXT.md). This ADR records the hard-to-reverse choices and the reasoning a future reader would otherwise ask "why?" about.
