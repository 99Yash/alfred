...
**Decision.** `mcp.call`'s `high` approval floor drops to `low` for a tool that satisfies two independent, durably-recorded conditions: its connection's endpoint is a built-in read-only protected resource (`BuiltInClientPolicy.readOnlyCatalog`, ADR-0094), AND the published catalog recorded that tool's own `annotations.readOnlyHint` as true (`mcp_catalog_revisions.read_only_hints`). No `mcp_tool_policy` row is required.
...
2. **The per-tool claim is PROJECTED at publication, not scanned at the gate.** `computeReadOnlyHints` writes `{ [remoteName]: boolean }` beside `descriptor_hashes`, so the gate resolves one tool by name.
3. **The risk gate re-reads the claim from persisted state.** It does not infer "this catalog is all reads" from the fact that the ADR-0095 admission gate exists.
...
- `readOnlyCatalog` is pinned in Alfred's own built-in registry. A server cannot set it.
...
- **A tool with no annotation.** An absent optional annotation carries no claim. `computeReadOnlyHints` records `false`, and the gate needs a `true`.
...
- **No test proves the branch table**, per repository policy. The compiler cannot reach a runtime tier decision, so the proof is `references/scratch/probe-structural-risk-downgrade.ts`, which drives `resolveMcpCallRiskTier` against a real database over eight rows and reads them all as designed: read-only endpoint with a published hint is the ONLY row that reaches `low`; a missing hint, an ordinary endpoint, a corrupt reviewed tier, and a stale catalog revision each hold `high`; and a reviewed row moves the tier in both directions. Being a scratch probe, nothing re-runs it — a later edit to `effectiveMcpRiskTier` has no gate behind it.
