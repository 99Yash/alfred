...
5. **No adapter returns raw bytes or full bodies.** A card carries only a bounded snippet in this slice. Scores, citations, and the opaque `expansionHandle` for live drill-down (#428) are #423's to define. Provider-specific action tools are never invoked by the fabric.
6. **The evidence card is internal and provisional.** `ContextEvidence` is not exported from the module barrel; it exists only so the empty result and the `ContextSource` return type are typed. #423 owns the canonical EvidenceCard contract and the packing rules; #429 extends media kinds. No consumer may build on the card yet.
...
- **`ContextEvidence` will churn.** It is provisional and module-internal; #423's canonical contract is expected to replace it outright.
