...
**The rule.** `BuiltInDefinition.readOnlyCatalog` marks a provider whose endpoint is a read-only protected resource. `McpRawClient` refuses the whole catalog refresh when any descriptor fails to assert `annotations.readOnlyHint === true` (`McpClientError` code `write_tool`). The policy is INJECTED, not looked up: the raw client knows nothing about the built-in registry, so `liveClientFactory` spreads `builtInClientPolicy(endpointUrl)` and passes the answer.

**Two readers, two names.** `builtInClientPolicy` returns this flag together with the ADR-0095 era pin, because those two answer one question — what does Alfred owe this endpoint that it does not owe an arbitrary one — and one call site spreads both. It stays OFF the `connections/mcp` barrel, like `builtInAuthorizationScopes`: its only caller is `liveClientFactory`, inside the directory. The one name on the barrel is `builtInReadOnlyResource(endpointUrl): boolean`, which the ADR-0096 risk gate reads. That gate asks a RESOURCE-trust question and has no business learning which protocol era Alfred negotiates, so it does not receive a client-configuration bundle to pick a field out of.
...
- **The registry is keyed on the stored ENDPOINT, not on `canonical_resource`.** `ensureServerDefinition` may retarget an endpoint that moves UNDER an unchanged resource. After such a move, `builtInReadOnlyResource` answers `false` for the stored row and the tool falls back to the `high` floor. That is the direction to fail in — keying on the resource would keep a downgrade alive for an endpoint that had moved — but the LOSS is silent. For GitHub it is loud instead, because the same miss also drops the era pin and the connection then fails admission on `x-mcp-header`. A future built-in that declares no such keyword would lose its read-only flag with nothing reporting it.
