...
**Dispatcher autonomy override.** **(Superseded 2026-08-13 — the check moved into `resolvePolicyMode`; the dispatcher now writes `const policyMode = await resolvePolicyMode(args.userId, toolName);`. The block below is the original shape. See the amendment at the end of this file.)**
...
**Amendment (2026-08-13) — the `system.*` autonomy short-circuit moves into `resolvePolicyMode`.** Decision 5 above puts the structural `if (integration === 'system') policyMode = 'autonomy'` check in the dispatcher, before `resolvePolicyMode`. The check now lives at the top of `resolvePolicyMode` itself (`packages/assistant/src/action-policies/resolve.ts`), ahead of the read order the ADR states (tool override → integration mode → user default) and ahead of the policy row read. The invariant is unchanged and widened: `system.*` is structurally non-gateable, and every caller gets that answer, not only the two dispatch call sites that hand-applied it.

The four failure modes the rationale names — a future user toggle, a missing default row, a botched migration, a policy-editor bug — are all data-layer failures. A check that precedes `getResolvedPolicy` survives all four identically, so the defense in depth is preserved. The seeded `integration_rules.system = { mode: 'autonomy' }` row stays the second line of defense. One property of the old placement is also preserved deliberately: a `system.*` resolution performs **zero** policy DB reads, which is what makes `packages/assistant/test/tool-runtime/dispatch/staging-machine.test.ts` and `would-gate.test.ts` DB-free.

Two reasons the location moves. First, `resolvePolicyMode` is now a public `@alfred/assistant/action-policies` specifier reachable from `@alfred/http`, `apps/server` and every assistant module, and `@alfred/http` is taking ownership of write paths that reach it; a new caller that does not know the rule would silently gate a `system.*` call. Second, the two hand-written homes had already diverged. `system.activate_workflow` is registered `riskTier: "high"`, and ADR-0069 makes `high` a hard approval floor that autonomy cannot override, so `dispatchToolCall` stages it — while the `toolCallWouldGate` scheduling hint returned `false` before it ever reached that floor. `dispatchLiveChat` therefore kept the call in the concurrent bucket, where it could stage a second approval card beside the serial one, which is the exact failure the hint exists to prevent. With the rule in the resolver, the hint falls through to the risk floor and answers `true`.

`system.*` remaining `autonomy` means the user's policy never gates a system tool. It does not mean a system tool never stages: the ADR-0069 tier floor is ORed with the policy mode and still applies.
