...
- **No consumer calls `searchContext` yet.** Two adapters are registered at boot (#424), but nothing reads the boundary until the model-facing tool lands (#426), so "a new source needs no consumer edit" is exercised only by the two built-in adapters, not by a real caller.
...
- **Each vector source embeds the query itself.** `searchContext` calls every registered source, and both built-in adapters invoke a primitive that embeds the query, so one boundary read costs one embedding call per vector source. The shared-embedding seam lands with the model-facing tool (#426), which is the first real caller; no production path reaches the boundary today, so this slice pays nothing. The direct combined caller (`skill-documentation-context`) embeds once and passes `queryEmbedding` to the primitives itself and is unaffected.
