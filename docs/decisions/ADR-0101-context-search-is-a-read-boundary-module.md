...
5. **No adapter returns raw bytes or full bodies.** A card carries only a bounded snippet (or an honest note when there is no text). The card's `score`, `citations`, and opaque `expansion` handle were defined by #423; the first adapters fill all three. Provider-specific action tools are never invoked by the boundary.
