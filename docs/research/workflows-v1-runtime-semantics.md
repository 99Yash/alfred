...
**Resolved (issue #530).** Every terminal write on `agent_runs` — the four commit branches plus `markRunFailed` — goes through `guardRunOwnership`, which takes the row lock and classifies under it, so a commit racing a mid-step cancel rolls back and reports a `run_already_terminal` skip instead of resurrecting the run. Because the commit rolls back, closure is the cancel path's job: a workflow that owes the client an ending implements `Workflow.onTerminal`, driven from `terminal-closure.ts` wherever the terminal write happens. The hook's context is a discriminated union on `outcome` (`"failed"` / `"cancelled"`) rather than two optional hooks, because the two renderings are not interchangeable — a cancel must not render a retryable error — and a workflow handling only one of them would otherwise compile. The chat workflow closes a cancelled turn as **complete**, carrying the run's last *committed* text: closure re-reads `agent_runs.state`, and a mid-step cancel is precisely the case where the in-flight step's accumulated text was rolled back, so a cancel inside the first assistant step closes an empty turn. That is deliberate — the alternative is persisting text no commit ever accepted — and it is *not* the same mechanism as the Redis stop path, which finalizes from inside the step body with live in-memory state; only the rendered status matches. The commit-time fence is in place — the "cancellation check immediately before dispatch" for already-staged effects is not, and neither is aborting the in-flight step body, so a cancel still pays for the model call already running.
