...
**Resolved (issue #530), except for aborting an already-running provider call.** Every terminal write on `agent_runs` — the four commit branches plus `markRunFailed` — goes through `guardRunOwnership`, which takes the row lock and classifies under it, so a commit racing a mid-step cancel rolls back and reports a `run_already_terminal` skip instead of resurrecting the run. Because the commit rolls back, closure is the cancel path's job: every workflow declares `closure: { kind: "none" | "client" }`, and a client-owning workflow supplies the exhaustive `onTerminal` hook driven from `terminal-closure.ts`. The chat workflow closes a cancelled turn as **complete**, carrying the run's last _committed_ text; its step-body finalizers check cancellation at the user-visible write boundary and re-check before arming memory, compaction, or title generation. The dispatcher likewise checks terminal state before staging, while the cancel post-commit path and a superseded executor both sweep late `pending` stagings and tear down their jobs, closing the autocommit race that the executor transaction cannot roll back. The remaining limit is physical interruption: a provider call already in flight when cancellation lands can still finish and be billed, but its result, success tail, approval row, and executor commit are suppressed or compensated.
