...
| 4   | Alternatives                                               | **PIVOT-ready**                  | `node:vm` is explicitly **not** a security boundary (disqualified). **quickjs-emscripten** (WASM QuickJS) is a _stronger_ network-isolation story by construction — no syscalls at all — with asyncify host fns, at a perf cost. Deno `--deny-net` is app-level, not OS-level.                                                                                                          |
...
ADR's network claim**: OS-enforced egress denial is _not_ achievable in an unprivileged Railway
...
- **#470** "Compile errors with node 22 on archlinux" — _closed_ 2024-05-19. Root cause: Node 22's V8 API
...
- **#500** "Compile error with node v23.0.0" — _closed_ 2024-10-22. Node 23 requires **C++20** (`"C++20 or
later required"` fatal error) plus removal of `v8::CopyablePersistentTraits` / `SetAccessor`. Resolved
  in later releases.
- **#531** "feat: Node 24 support" — _closed_ 2025-05-20 (i.e. 6.0.0).
- **#534** "Node 18, 20 backwards compatibility" — _closed_ 2025-08-16. Confirms the project **dropped
  Node 18/20**; 6.0.1+ floors at Node 22.
- **#548** "Prebuilt binaries for version 6.1.0 incompatible with Ubuntu 22.04" — _closed_ 2026-03-11.
...
- **#528** "Electron 35.2.0 (Node 22.14.0) — Build issue — C++ 20" — _open_ (Electron-specific; not our case).
...
Also, whether the _container_ runtime's own seccomp profile even permits a nested `seccomp()`/`prctl`
...
- Railway station "Docker in Docker" thread confirms nested containerization / runtime installs are
  ...
  but the _runtime_ container is unprivileged; the Dockerfile controls image contents, not kernel
...
- **seccomp-bpf socket-family deny** — **the one unprivileged-viable path**, but fragile under Node's
  thread pool and dependent on Railway not blocking nested seccomp; treat as _best-effort
  defense-in-depth_, not a guarantee.
...
the injected code has nothing to send and no way to send it. That is ADR-0087's _actual_ containment
...
- Invocation variants on a `Reference` to a host function:
  ...
  - **`reference.applySyncPromise(receiver, args, opts)`** — the key one: the **host** function may
    ...
    then receives the resolved value. This is only callable from a _non-default_ isolate (i.e. exactly
...
> (even inside a worker) is not an isolation story. Only viable as the _transport_ (the forked worker
> that _hosts_ `isolated-vm`), never as the sandbox itself.
...
- **Isolation by construction:** "By default, no host functionality is exposed to code running inside
  ...
  filesystem at all** — the WASM sandbox cannot even _name_ `socket`/`connect`. This is a _strictly
  stronger network-isolation story than `isolated-vm`_: with `isolated-vm` the isolate is
...
- **No synchronous RPC / host-callback primitive** exists for code _inside_ the sandbox to call back
...
- Therefore, to implement `load(handle)`/`broker.read`, the only options are: **(a)** ship all needed
  private data _into_ the sandbox at creation via `writeFiles` (i.e. copy the parked Gmail/GitHub/Railway
...
ADR-0087's containment is: _the sandbox holds no credentials and private data is fed by host functions
running in the trusted parent, so data never leaves Alfred's infra._ Vercel Sandbox is remote, so to
...
through a third party"). What Vercel documents _better_ than Freestyle: **"Sandboxes run on Vercel's
...
data custody is.** A V8 escape in a _local_ isolate compromises a Railway worker that already holds no
creds; shipping the private data to a third party is a _data-location_ regression that a stronger
...
with a **best-effort seccomp-bpf socket-deny** attempt as defense-in-depth (and _measure_ whether it
...
that is almost certainly acceptable for handle-reshaping work. Consider prototyping _both_ `load()`
