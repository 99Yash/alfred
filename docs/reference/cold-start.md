...
5. **extract-facts** — `extractColdStartFacts({ signals, research })`. Cheap-tier (`route("cheap").model()` + `meteredGenerateObject`) converts the summary into structured `{ key, value, confidence, rationale }` proposals (ADR-0019's two-stage extract tail, unchanged).
