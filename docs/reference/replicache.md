...
4. Add the matching server-side mutator in `packages/http/src/sync/server-mutators.ts` — write against the supplied `tx` (so it commits inside the push handler's outer transaction) and bump `row_version`. Pokes fire generically from the push handler after commit.
5. Add a fetcher to `ENTITY_FETCHERS` in `packages/http/src/sync/entities.ts` returning `{ id, rowVersion, serialized }` per row (`pull.ts` only consumes that row shape). The CVR snapshot shape (`Partial<Record<IDBKeys, ClientViewMap>>`) is generic — no `cvr.ts` change needed.
