...
**Classifier rubric.** The per-category rules live in `SYSTEM_PROMPT` in `packages/api/src/modules/triage/classify.ts`, which is the only source of truth for them. They are deliberately not restated here: the rubric is the behavior, so a prose copy competes with the prompt the model actually sees and the copy always loses. Read the prompt.
