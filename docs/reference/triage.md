...
1. A Gmail ingestion job inserts a fresh `documents` row. The realtime path is `gmail.poll_recent` (pub/sub → `messages.list?q=newer_than:5m`, ADR-0037); the catch-up path is `gmail.poll_history` (5-min sweep → `history.list` from the stored cursor; the sweep polls any credential whose last sync is older than `GMAIL_POLL_SWEEP_STALE_AFTER_MS`, one minute short of the cadence, so a dead push costs one cadence and not two). Both call into the same `persistMessage` helper so dedup behaves identically.
