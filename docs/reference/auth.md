...
- **Connect** — `POST /api/integrations/sentry/connect` takes the token and the organization slug. `sentryValidateToken` runs two `org:read` reads: `GET /organizations/{slug}/` gives the identity the credential stores as `accountId`, and `GET /organizations/{slug}/sentry-app-installations/` gives the installation whose `app.slug` equals `SENTRY_INTEGRATION_SLUG`. The installation `uuid` is persisted as `installation_id` on `integration_credentials`. An internal-integration token cannot list organizations, so the slug is required. A token that Sentry accepts, in an organization where the integration is not installed, is a `SentryInstallationNotFoundError` (502), not an auth failure.
- **Env** — `SENTRY_INTEGRATION_SLUG` (the integration's slug; the connect route returns 503 while it is unset) and `SENTRY_WEBHOOK_CLIENT_SECRET` (the same integration's Client Secret). Both are optional so the server boots before the integration exists.
...
- **Owner** — `installation.uuid` at the payload root is joined to the credential through `findActiveCredentialByInstallationId({ provider: "sentry", installationId: uuid })`; a delivery for an installation nobody connected is acknowledged and dropped. Subscription health is `hasActiveInstallationCredential`, with recovery `connect`, after the secret check above.
