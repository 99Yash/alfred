...
**Internal integration, token paste** (ADR-0093 registry entry, #563). Sentry's public OAuth is for *public* integrations only, so the user pastes the token of an internal integration in their own organization. Code in `packages/integrations/src/sentry/`:

- **Connect** — `POST /api/integrations/sentry/connect` takes the token and the organization slug. `sentryValidateToken` runs one `org:read` read, `GET /organizations/{slug}/`, and the credential stores the organization as `accountId`/`accountLabel`. An internal-integration token cannot list organizations, so the slug is required. The credential stores **no** `installation_id`: an integration token gets 404 "Could not find requested organization" from `GET /organizations/{slug}/sentry-app-installations/`, because Sentry resolves that endpoint through the caller's memberships and the integration's proxy user has none (verified live 2026-09-06 on `sentry.io` and `de.sentry.io`; the same call from a logged-in browser session returns the list).
- **Env** — `SENTRY_WEBHOOK_CLIENT_SECRET`, the integration's Client Secret. Optional so the server boots before the integration exists. The connect route needs no env.
- **Webhooks** — the hook URL on the integration is the **prod server domain only**: `https://<server>/webhooks/inbound/sentry`. The `sentry` descriptor in `packages/assistant/src/connections/ingress/sentry.ts` calls `verifySentryWebhookSignature`, which checks `Sentry-Hook-Signature` (HMAC-SHA256 hex over the raw body, keyed with `SENTRY_WEBHOOK_CLIENT_SECRET`) before any parse; while the secret is unset every delivery is rejected with 401, the log names the missing env var (not a mismatch), and subscription health reads unhealthy with recovery `none`. The event type is `<Sentry-Hook-Resource>_<action>`; a pair that is not in `EVENT_SOURCE_ENTRIES` (`installation.*`, `comment.*`, `metric_alert.*`, and any new action) is ignored as `unsubscribed`. Nothing consumes `installation.deleted` yet, so an uninstall leaves the credential `active` until a read fails.
- **Dedup** — Sentry has no stable delivery id (`Request-ID` changes on each retry), so the receipt key is synthetic per event type, and the key function switches on the projected type exhaustively: `error_created:<event_id>`, `event_alert_triggered:<event_id>:<triggered_rule>` (or `event_alert_triggered:<event_id>` when the rule label is absent), `issue_created:<issue.id>`, `seer_pr_created:<run_id>`. The repeatable issue transitions (`resolved`, `unresolved`, `assigned`, `archived`) append the first 16 hex of the body sha256, `issue_resolved:<issue.id>:<digest>`, because `(issue, action)` alone would file a second resolve two days later as a duplicate of the first; a retry re-sends the queued body, so its digest matches, and a retry whose issue changed between attempts is the accepted duplicate. `getIdPath` (`@alfred/contracts`) collapses string and integer ids to one string so a key never depends on Sentry's serialization.
- **Owner** — the one active `sentry` credential, through `findSoleActiveCredential({ provider: "sentry" })`. The body names no organization and the connect flow cannot learn the installation uuid, but one Client Secret is one internal integration in one organization, so a verified signature already identifies the sender. No active credential: the delivery is acknowledged and dropped. More than one: the deployment has outgrown a single secret, and the delivery is dropped with an error log rather than attributed by guess. Subscription health is `hasActiveCredential`, with recovery `connect`, after the secret check above.
- **Seer** — `parseSeerPullRequestsCreated` is the boundary parse for a stored `sentry.seer_pr_created` receipt (one Autofix run, the issue `group_id`, one pull request per repository). Nothing else interprets Seer bodies; the verification rung (#567) reads through it.

**Registering the integration (one-time).** In the Sentry organization, create an internal integration (Settings → Developer Settings → Custom Integrations → Internal). Give it `org:read`, `project:read`, `event:read`, and `issue:read` (the passthrough tier is read-only). Set the webhook URL above, and subscribe it to `issue`, `error` (Business plan and up), and `seer` resources; `event_alert` deliveries arrive through an alert rule whose action targets the integration. Copy the Client Secret into `SENTRY_WEBHOOK_CLIENT_SECRET` on Railway (server service), then create a token on the integration and paste it in the web form with the organization slug.
