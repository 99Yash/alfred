...
- **Webhooks** — the hook URL on the integration is the **prod server domain only**: `https://<server>/webhooks/inbound/sentry`. The `sentry` descriptor in `packages/assistant/src/connections/ingress/sentry.ts` calls `verifySentryWebhookSignature`, which checks `Sentry-Hook-Signature` (HMAC-SHA256 hex over the raw body, keyed with `SENTRY_WEBHOOK_CLIENT_SECRET`) before any parse; while the secret is unset every delivery is rejected with 401. The event type is `<Sentry-Hook-Resource>_<action>`; `installation`, `comment`, and `metric_alert` resources are ignored, and a resource/action pair that is not in `EVENT_SOURCE_ENTRIES` is ignored as `unsubscribed`.
- **Dedup** — Sentry has no stable delivery id (`Request-ID` changes on each retry), so the receipt key is synthetic per resource: `error:<event_id>`, `event_alert:<event_id>:<triggered_rule>` (or `event_alert:<event_id>` when the rule label is absent), `issue:<issue.id>:<action>`, `seer:<run_id>:<action>`. `payloadIdAt` collapses string and integer ids to one string so a key never depends on Sentry's serialization.
- **Owner** — `installation.uuid` at the payload root is joined to the credential through `findActiveCredentialByInstallationId("sentry", uuid)`; a delivery for an installation nobody connected is acknowledged and dropped. Subscription health is `hasActiveInstallationCredential`, with recovery `connect`.
