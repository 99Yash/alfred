...
Model selection: `getBossModel()`, `getSubAgentModel()`, `getCheapModel()`, `getWebSearchModel()`, `getChatModel()` from `@alfred/ai/provider`. Do not call AI SDK provider functions directly from route handlers, and do not hardcode string model IDs in type positions — `LanguageModel` is a union.
