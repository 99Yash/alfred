...
- **`exactOptionalPropertyTypes` is ON (#552), so the declaration says whether a field tolerates `undefined`.** `k?: T` means *absent or a `T`* — never present-and-undefined. `k?: T | undefined` means the field also accepts an explicit `undefined`. Pick deliberately when you declare an optional field:
  ...
  - Keep the narrow `k?: T` when *absence* is load-bearing, and make the caller omit the key with a conditional spread — `{ ...(x ? { k: x } : {}) }`. The case the flag now catches: a Drizzle insert walks `Object.keys`, so a present `undefined` binds `NULL` where an absent key emits the column `DEFAULT` (`mcp/broker.ts` `insertInvocation`).
  ...
  - The flag only checks `src`. Test files are compiled by nothing (see [TypeScript conventions](./typescript.md)), so a present-`undefined` bug in a fixture is invisible — don't treat a green `check-types` as coverage of test code.
