...
- **`exactOptionalPropertyTypes` is ON (#552), so the declaration says whether a field tolerates `undefined`.** `k?: T` means *absent or a `T`* — never present-and-undefined. `k?: T | undefined` means the field also accepts an explicit `undefined`. Pick deliberately when you declare an optional field:
  ...
  - When the object literal contains *nothing but* the conditional, drop the wrapper instead of spreading into an empty object — `f(x ? { k: x } : {})` — or oxlint's `no-useless-spread` will (correctly) object.
