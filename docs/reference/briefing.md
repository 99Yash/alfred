...
> **Changed from m10.** The m10 briefing read Gmail-only from the local DB and skipped `requireScopes`. The live model adds **calendar anchoring** and **compose-time reconciliation**, so briefing is no longer local-DB-only: **`GOOGLE_SCOPE.calendar.readonly` (`calendar.readonly`, `@alfred/contracts` `google-scopes.ts`)** lives in `GOOGLE_FEATURE_SCOPES.briefing` for read-only gather. Interactive Calendar connects now grant `calendar.events` for read/write tools, and read paths accept either scope so older readonly credentials continue working. Existing users re-consent on next refresh/reconnect; `include_granted_scopes=true` merges it into the existing grant. Reconciliation reads ingested integration state where Alfred has it. Per the source-availability contract above, missing scope or unavailable provider state **degrades to surface-as-live**, never to inferred closure.

- `packages/integrations/src/google/oauth.ts` exposes `GOOGLE_FEATURE_SCOPES` (`briefing` / `triage` / `reply_draft`, defined in `@alfred/contracts` `src/google-scopes.ts` and re-exported) + `scopesForFeatures(features?)`. `ALL_GOOGLE_SCOPES` is the union of every feature; `DEFAULT_GOOGLE_SCOPES` aliases it. The old PUBLIC/RESTRICTED scope tiering was removed when ADR-0044 was amended (2026-06-08) to grant-all — Alfred runs single Production-unverified, so there is no public-app verification surface to minimize.
