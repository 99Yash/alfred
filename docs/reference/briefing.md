...
> **Changed from m10.** The m10 briefing read Gmail-only from the local DB and skipped `requireScopes`. The target model adds **calendar anchoring** and **compose-time reconciliation**, so briefing is no longer local-DB-only: the existing **`CALENDAR_READONLY_SCOPE` (`calendar.readonly`, `oauth.ts`)** is added to `GOOGLE_FEATURE_SCOPES.briefing` (which today carries only `gmail.readonly`). This reuses the scope the `calendar` feature already grants rather than introducing the narrower `calendar.events.readonly` — see ADR-0048 decision D for why. Existing users re-consent on next refresh/reconnect; `include_granted_scopes=true` merges it into the existing grant. Reconciliation reads ingested integration state where Alfred has it. Per the source-availability contract above, missing scope or unavailable provider state **degrades to surface-as-live**, never to inferred closure.
