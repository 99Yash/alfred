...
> **Changed from m10.** The m10 briefing read Gmail-only from the local DB and skipped `requireScopes`. The live model adds **calendar anchoring** and **compose-time reconciliation**, so briefing is no longer local-DB-only: **`CALENDAR_READONLY_SCOPE` (`calendar.readonly`, `@alfred/contracts` `google-scopes.ts`)** lives in `GOOGLE_FEATURE_SCOPES.briefing` for read-only gather. Interactive Calendar connects now grant `calendar.events` for read/write tools, and read paths accept either scope so older readonly credentials continue working. Existing users re-consent on next refresh/reconnect; `include_granted_scopes=true` merges it into the existing grant. Reconciliation reads ingested integration state where Alfred has it. Per the source-availability contract above, missing scope or unavailable provider state **degrades to surface-as-live**, never to inferred closure.
