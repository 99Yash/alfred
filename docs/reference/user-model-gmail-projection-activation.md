...
2. **Gmail observation backfill has been run at scale** for the target user
   ...
   node dist/scripts/backfill-gmail-observations-committed.js --emails=yash.k@oliv.ai
   node dist/scripts/backfill-gmail-observations-committed.js --emails=yash.k@oliv.ai --commit
   ```
...
```bash
railway ssh -s server
node dist/scripts/project-user-model-gmail-shadow-committed.js \
...
- Projection/activation script: `apps/server/src/scripts/project-user-model-gmail-shadow-committed.ts`
  (dist: `dist/scripts/project-user-model-gmail-shadow-committed.js`).
