...
| collapse a whitespace run to one space for **display** text                                   | `collapseWhitespace(text)`                                                                                                                                                                                                                                                                                         | `@alfred/contracts`           | `.replace(/\s+/g, " ").trim()` — **drift check bans the raw idiom**. A key, hash or classifier input keeps its own fold plus a `// drift-ok:` reason.                        |
...
- It is **not** for a value that builds a key, a hash, a dedup token or a
  classifier input. This fold may change whenever a display improves; a stored
  identity may not. Those sites keep a private fold and a `// drift-ok:` reason.
