...
**Decision.** The boss must never be relied on to (a) recall an external query language's exact syntax from its weights, or (b) do timezone-correct "today" math by hand. Two structural rails enforce this. **(1) The user's IANA timezone is a first-class field on `ToolExecuteContext`** — the dispatcher resolves it once (from the `"timezone"` pref, falling back to UTC; reusing chat's per-run snapshot when present) and hands it to *every* tool, so any tool that turns a relative window into concrete bounds anchors "today"/"this week" on the user's **calendar day**, not the server's UTC day. **(2) Time-relative filters are structured fields resolved server-side, and the free-form escape hatch is validated, not trusted.** For `github.search_pull_requests`: `closedWithinDays`/`createdWithinDays`/new `mergedWithinDays` (`N=1` = today in the user's zone, `N=7` = the last 7 calendar days) compile to `merged:>=<tz-local date>` in the handler; the free-form `query` is whitelist-checked at schema time — an **invented qualifier** (`merged-by:`, `closed-by:`) or one that **collides with a structured field** (`is:`/`author:`, or a date qualifier whose `*WithinDays` field is also set) is rejected as `invalid_input`, so the boss gets an actionable signal and retries instead of shipping a confident wrong answer.
