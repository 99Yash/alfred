...
**Why.** The boss's "prepare a PDF" misuse (#267) exposed that there is *no* produce-artifact path — it reached for a read-text tool and poisoned persistence. The owner wants this as a first-class capability (PDF/slides/sheets), and judged Dimension's inline-editable UX the better target ("user can see it inline and suggest edits"). Recon of `-dimension-ai-web` (`ARTIFACT_SIDEBAR_PLAN.md` + `apps/web/.../artifact-sidebar/`) confirms the proven shape: agent-authored HTML/markdown/rows in self-built renderers, stored locally, streamed into a sidebar — not cloud preview URLs. Deferred because it doesn't touch the poison bug and is a real epic (renderers, streaming, a new synced artifact entity, create-artifact tools).

**The shape (when picked up).** A synced `artifact` entity (Replicache, per the add-an-entity recipe) carrying type + content + run scope; create-artifact tools (`create_slide`/`create_pdf_page`/`create_spreadsheet`/`create_document`-style) the boss calls; an `artifact-sidebar` surface in `apps/web` borrowing renderers from `-dimension-ai-web`; R2 for any heavy binary (reusing the ADR-0065 file infra). The boss rubric (ADR-0071 #4) bridges until then: when asked for a downloadable artifact it can't yet produce, deliver a live shareable Google Sheet/Doc link (which already works) rather than failing.
...
**Cross-ref.** Sibling to ADR-0074 (that is invocation breadth; this is output rendering). Bridged by ADR-0071's "share a live Doc/Sheet link" rubric until built. Reuses ADR-0065 (R2 file infra) and the Replicache entity recipe. UX target: `-dimension-ai-web` artifact sidebar.
