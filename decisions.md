...
**Decision.** Everything a briefing writer says about the user's day flows through **three layers**, and `@alfred/contracts/briefing-signals.ts` owns the boundary between them: **(1) source evidence** — the raw, provenance-bearing gather items (a Gmail thread, calendar event, GitHub activity row), each addressable by a `BriefingReference` token, validated at its owning boundary, never authored by an LLM; **(2) typed context signals** — a small, closed vocabulary (`BRIEFING_CONTEXT_SIGNAL_KINDS`: `closed_work_loop`, `open_work_loop`, `shipping_momentum`, `job_search_event`, `recurring_machine_noise`, `integration_access_gap`) derived *deterministically or by a bounded projection* from Layer 1, each carrying its evidence back to Layer 1; **(3) generated prose** — the warm paragraph the writer emits, which *consumes* Layer 2, invents no durable facts, and is deliberately **not a type** (prose is ephemeral by construction). This makes the standing tenet — **deterministic core, prose only at the edge** (ADR-0067/0080) — structural for briefings: a writer may phrase evidence warmly, but warmth is Layer 3, derived at query time, never persisted. Two invariants are made machine-checkable, not just asserted: **(a)** each signal declares a `durability` — `grounded_projection` (object-state facts with positive evidence, eligible to back a *bounded* projection) vs `ephemeral_query_time` (mood/momentum/warmth aggregates, render-time only) — and `isEphemeralBriefingSignal` names the class that MUST NOT be stored; **(b)** `briefingContextSignalSchema` requires non-empty `evidence` ("no grounding, no row," ADR-0080 §1), derives `durability` from `kind` (never carried on the wire, so an ephemeral signal can't be spoofed as persistable), and bounds evidence fan-out + the phrasing hint. The **memory write policy**: briefing signals are a namespace *disjoint* from `FACT_ONTOLOGY` (a contract test pins the empty intersection), so a signal is never a `user_facts.key`; briefing work must not promote email/document metadata (subject/sender/message-id/dates) or a third party's attributes (a recruiter's company, a contact's city) into the user's identity/org facts, and must not persist a vague observation (shipping mood, motivation, warmth) as a durable fact. Durable identity/org writes stay the job of the ADR-0080 projection (`PROJECTION_IDENTITY_KEYS`, `identity-affiliation.ts`) governed by aboutness-by-construction; this slice ships the contract + tests only — no composer wiring.

**Why.** The failure this forecloses is documented and expensive: the `yash.k@oliv.ai` mailbox drove `employer="Weekday"` (a recruiter) and 900+ harvested company/title/metadata rows (`.lessons/user-facts-document-metadata-noise.md`), because an LLM read of a document was allowed to write a durable identity fact with no aboutness check. Briefings are the *next* place an agent phrases things about the user warmly, so they are the next place that leak can reappear — via a "shipping mood" or "job-hunt momentum" line quietly round-tripping into `user_facts`. Naming the three layers and the durability gate **before** any writer is wired means future briefing work inherits the guardrail structurally instead of re-discovering it. A typed vocabulary (over free-form strings the composer invents) also gives the eventual composer a crisp, testable surface to reason over and keeps `email`/`GitHub`/`calendar` signal semantics in one place rather than smeared across prompts. The taxonomy is intentionally small and additive — later slices grow it, they do not fork it.

**Alternatives.** (a) **Skip the typed layer; let the composer read gather and emit prose directly** — rejected: that is exactly the shape that let the payment-reconciliation set-op (ADR-0048/#256) and the identity hallucination through; no typed seam means no place to pin "evidence-required" or "never persist momentum." (b) **Build the signals as a new observation kind / projection now** — rejected as premature: this slice is the *contract*, and wiring a projection is a consumer that belongs to a later #218 slice; shipping the vocabulary + policy first is the cheapest correct step and unblocks the composer work without committing schema. (c) **Enforce the memory write policy with a new runtime guard in this package** — rejected: enforcement already lives where it has document context (`fact-policy.ts` Layer-2 gate, ADR-0079) and where identity is projected (ADR-0080); duplicating it here would create a second adjudicator. This module *names* the policy and proves the disjoint-namespace invariant; the existing gates enforce it. (d) **Carry `durability` on the signal payload** — rejected: it would let a caller mark an ephemeral signal persistable; deriving it from `kind` is unspoofable.

**Cross-ref.** Sits under the #218 user-model spine; consumes the ADR-0048 open-loop model + source-availability contract (`integration_access_gap` is the "couldn't verify → surface as live" case; `closed_work_loop` requires positive evidence), ADR-0064 presentation-layer attention demotion (`recurring_machine_noise`), and the ADR-0067 observation log / ADR-0080 identity projection it must never bypass. `BriefingReference`/`parseBriefingReference` (ADR-0049) are the Layer-1 provenance handles. Enforcement homes: `fact-policy.ts` (ADR-0079), `PROJECTION_IDENTITY_KEYS` (ADR-0080). Contract + tests: `packages/contracts/src/briefing-signals.ts`, `packages/api/test/contracts/briefing-signals.test.ts`. **Deferred:** composer/gather wiring that emits these signals; any bounded projection backing `grounded_projection` signals; growing the taxonomy.
