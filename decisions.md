...
**Sequencing.** ADR + plan first (this), then incremental, Gmail-first, each phase shadow-built + validated: **P0 ✅ built 2026-06-23** (branch `feat/user-model-substrate-p0`, migration `0050`, additive) — schema + `@alfred/contracts/user-model.ts` registries + projection run/cursor tracking + Replicache projection-sync state; stable layer `entity_nodes`/`entity_identities` (content-addressed ids, unversioned) vs versioned `entity_profiles`/`entity_edges`/`entity_co_occurrence`, with the locked constants (`FAN_OUT_CUTOFF=12`, `PROMOTION_THRESHOLD=2.0`, `PROMOTION_MIN_OBSERVATIONS=3`, `PROMOTION_MIN_FAMILIES=2`, `SOURCE_WEIGHTS`). New tables are named `entity_nodes`/`entity_edges` (not `entities`/`entity_relations`) to coexist with the legacy memory-module graph through the shadow phase; `user_facts` subject-binding + active-version views deferred to P4/P1 respectively · **P1** Gmail end-to-end + validate vs today, including the `engineering@oliv.ai` group/unscored fix · **P2** GitHub (commit-email bridges + wire stable-ID object_relations) · **P3** Calendar + conditional Google Workspace Directory (new ingestion, highest first-party people signals; verify `https://www.googleapis.com/auth/directory.readonly` is grantable and the `oliv.ai` admin-side profile/contact sharing toggle is enabled before adding it to `GOOGLE_FEATURE_SCOPES`) · **P4** fact ontology (subject-binding + purge; migrate all non-proposed user-authoritative facts into user observations; replay-derived status/confidence) · **P5** consumer cutover (triage/briefing/meeting-prep + re-scoped 0066) + Replicache delta validation + drop legacy.

**Parked / open (decide during build).** Provisional-entity auto-merge aggressiveness once a bridge appears; whether group rosters (`member_of`) are reliably inferable from list-addressed mail or need Directory/explicit source; final active-view names; Google Workspace Directory scope/admin-sharing verification; third-party enrichment is post-v1, proposed-only + cited, layered on ADR-0057's existing significance-gated enrichment path, not a new dependency; evalite coverage for the projection. Locked in P0: `FAN_OUT_CUTOFF = 12`, `PROMOTION_THRESHOLD = 2.0`, `PROMOTION_MIN_OBSERVATIONS = 3`, `PROMOTION_MIN_FAMILIES = 2`, `SOURCE_WEIGHTS`, and deterministic merge survivor/seed ordering. Plan: [docs/plans/multi-source-user-model-v1.md](./docs/plans/multi-source-user-model-v1.md).
