...
| Memory store | **Postgres substrate over a graph DB**; build the memory *intelligence* (extraction, contradiction, lifecycle), don't buy the *store*. Graph as a **model** (`entities`/`entity_relations` + pgvector), not a graph **engine** (Neo4j/FalkorDB) — the engine's advantage is a node-scale Alfred (single-user) never hits, and adopting Zep/Mem0 moves the source of truth off Postgres, breaking the Replicache memory-changelog surface + ADR-0038 privacy posture. Borrow Mem0's `ADD/UPDATE/DELETE/NOOP` taxonomy as logic over `proposeFact`/`supersedeFact`. Revisit only on multi-tenant-at-scale (ADR-0058) |
...
| Standing instructions | Durable user-stated **biases on existing pipelines** (triage/briefing/meeting-prep) — "tag X urgent", "stop nagging me about Ben", "keep recruiters out of my brief"; **not** workflow-authoring (that's ADR-0017/0027). **One central store** (`user_facts`, `key="standing_instruction"`, `schemaVersion:2` = `directive` prose always + `target?` + `enforcement?`; `action`/`surface` dropped) + **one retriever** (`getRelevantInstructions(context)`, inject-all-active behind a context interface). **Prose-first application** — agent consumers (chat, briefing compose, meeting-prep, every future feature) apply the `directive` by judgment; a thin **deterministic `enforcement` carve-out** (registry, keyed on "needs a hard guarantee") is consumed only by triage classify + briefing gather. Model never bends the category; an **explicit user instruction may** (`force_category`, **every override logged**). Conflict gated at capture (structural in the tool, semantic by the boss; offline → `proposed`). Precedence = specificity-then-recency; protective security floor beats a user down-rank, never an up-rank. Minimal `/settings` review panel in scope. Generalizes the ADR-0058 suppression slice (ADR-0060) |
