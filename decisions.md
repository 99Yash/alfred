...
1. **Build it right, no interim rubric hack.** An intrinsic-only rubric is *structurally* incapable of the directional call (a valuable stranger and a low-obligation-to-me senior person tie on every email-alone signal). Patching the rubric again would whack-a-mole against principles-over-exemplars. The fix is the missing signal, not more clauses.
...
7. **P4 split along the autonomy seam; P3 + P4a pulled ahead of the suppression slice.** **P4a (now):** passive-capture extraction as a standalone job over the *already-ingested* `documents` + calendar → populates the graph + first significance pass; needs neither the run-scoped autonomy override nor cold-start (the existing single user is already connected under watcher-approved autonomy). **P4b (deferred):** onboarding-time header-level seeding for *new* users, gated on the unbuilt run-scoped autonomy override (v2.1). This sequencing reverses the locked plan ordering: failure A is the user's live pain, and vertical slice 1 (Ben Book suppression) explicitly left failure A out — the per-sender suppression there is an *override on top*, not the root fix.

**What this amends / builds on.**

- **ADR-0050 D1** — un-defers and builds the personal-relevance primitive; the directional resolver *is* D1.
...
The first implementation overloaded the extraction marker for capture and applied increments *after* marking, so a batched-apply failure permanently dropped a whole run's increments (mark-then-apply = at-most-once, undercount on crash). Two fixes:
...
1. **Prose-first, not an effects enum (the redirect).** The first cut enumerated a symmetric per-consumer `effects` table; the user rejected it as forcing us to "keep up" per feature/integration. The spine flips: the resolved **`directive` prose is the universal carrier**, and the default application is *the consumer's own model reading the directive and applying it by judgment* — including flagging conflicts / counterproductive effects. A new agent-driven feature gets instruction-awareness for free by calling the retriever; it never extends a contract. Typed effects survive only as the carve-out below.

2. **Deterministic enforcement is a carve-out keyed on "needs a guarantee," not on feature count.** "Stop nagging me" injected as a prompt line is a suggestion the model can ignore — the original Ben-Book bug *was* a prose-fails-silently bug, and the cheap, high-volume triage classifier (ADR-0051 keeps it context-free) cannot afford per-email judgment. So a thin typed `enforcement` projection (`block/force_todo_suggestion`, `force_category{category}`, `exclude/include_briefing_priority`) is applied deterministically by **triage classify + briefing gather**. It is a **registry**: a *future* consumer that needs a hard guarantee registers its own enforcement, but ordinary new features use prose-pull. The shipped slice's `effects` enum *is* this layer, scoped down.
...
4. **Category override allowed — but the model never bends it.** Refines the "category stays honest" rule (ADR-0058 slice / ADR-0059). The *classifier* still never invents significance; an **explicit user instruction** may re-stamp the category, via the deterministic override layer (`force_category`), *after* honest classification — exactly like the existing security override-floor. The distinction is authority (the user) vs hallucination (the model). **Every override logs an audit breadcrumb** (`overriddenByInstruction:{factId,from,to}`) — a force-stamped category must never be traceless.
...
8. **Precedence at apply-time: specificity-then-recency, with a safety carve-out.** Multiple matching instructions → most-specific `target` wins (`sender_email`/`person` > `sender_domain` > `category` > `topic`), newest breaks ties. `enforcement` is authoritative over the model's intrinsic judgment for the dimension it touches. **The protective override-floor in `classify.ts` (force-`urgent` on exposed-secret / account-compromise) outranks a user *down-rank* but never a user *up-rank*** — a mute can't bury an irreversible-harm alert; an elevate stacks. Named, floor-only, logged.
...
**Open.** Base-demand weights + the two band cutoffs (seed by judgment, tune from the prod distribution — shared tuning surface with ADR-0057/0059 weights). Recurrence window vs. a short lookback query (does the 10× alarm span windows?). The ambient-tail floor (may recurrence demote `urgent`→`normal`, and never below? exposed-secret `urgent` stays pinned demanding). Eval coverage — an attention-scoring case in the `@alfred/api` evalite lane once the formula stabilizes. Plan: [docs/plans/triage-attention-demotion-v1.md](./docs/plans/triage-attention-demotion-v1.md).
...
**Alternatives.** (a) **Keep ADR-0064's separate attention axis** (honest category + dimmed rail) — rejected: the chip still reads `action_needed` on routine asks, which is exactly the noise; and the dimension recon shows even dimension's `action_needed` was noisy (its edge was context-driven *surfacing*, which we can't fully match and whose tagging output we wouldn't want to copy). (b) **Two-axis content×lifecycle state machine** (dimension's model) — rejected: dimension-package mirroring; the 10-label set + significance-weighting carries the value at single-user scale, and a second column breaks ADR-0048's one-tag invariant. (c) **Directional/outbound `awaiting_reply`** — rejected: handoff misread; dimension didn't tag bare sent-mail. (d) **Notification / quiet-hours gating** for the "2 AM" complaint — out of scope: that complaint was *category* not timing ("I can be asleep"), and tagging is already real-time (`ingest→tag` ≈ 0 on prod — no latency bug). (e) **Pure deterministic rules** for the default — rejected: free-text email can't be classified by rules alone; determinism lives in the *signals that constrain* the LLM, with user overrides on top ("good deterministic defaults, flexible override"). (f) **Reading dimension's source** to copy its logic — explicitly off-limits (Yash's call; it's his build + a portfolio artifact); recon from the labeled inbox only.
...
**Parked / open (decide during build).** Provisional-entity auto-merge aggressiveness once a bridge appears; whether group rosters (`member_of`) are reliably inferable from list-addressed mail or need Directory/explicit source; final active-view names; Google Workspace Directory scope/admin-sharing verification; provision `ENTITY_ID_NAMESPACE` before P1 writes stable ids; third-party enrichment is post-v1, proposed-only + cited, layered on ADR-0057's existing significance-gated enrichment path, not a new dependency; evalite coverage for the projection. Locked in P0: `FAN_OUT_CUTOFF = 12`, `PROMOTION_THRESHOLD = 2.0`, `PROMOTION_MIN_OBSERVATIONS = 3`, `PROMOTION_MIN_FAMILIES = 2`, `SOURCE_WEIGHTS`, and deterministic merge survivor/seed ordering. Plan: [docs/plans/multi-source-user-model-v1.md](./docs/plans/multi-source-user-model-v1.md).
...
**Decision.** A tool's *surface* (name, description, schema) must honestly represent what its API can and cannot do, and the boss must **attempt the closest real capability or state its limit — never silently narrow a task nor bail to the user for something it can resolve.** Concretely: **(1) GitHub search covers what its endpoint covers.** `search_pull_requests` is implemented on GitHub's `/search/issues` (issues *and* PRs); it is generalized to `github.search({ q, type: 'issue'|'pr'|'both', … })` with the validated structured surface and qualifier whitelist of ADR-0068 **extended to issue qualifiers** (`is:issue`, `label:`, `state:`); the `@me`→login server resolution is preserved (installation tokens aren't a user). **(2) Per-item detail is a typed fetch-by-number**, not a query: `github.get_pull_request` / `github.get_issue` (`owner`/`repo`/`number`) return what search structurally cannot — `additions`/`deletions`/`changed_files` for a PR (the #222 LOC need; the boss fans out over search hits and totals, capped + `log()`-ed on truncation) and body/comments for an issue. **(3) `drive.export_file` is an honest read-in tool**: its purpose (pull a Google-native file's *text* into the agent's context to reason over) is stated, its `mimeType` is bounded to a text-export allowlist (single source in `@alfred/contracts`), and a binary `mimeType` is redirected with a teaching message — "*I read files in as text; producing a downloadable PDF/slides for you is a separate capability.*" **(4) A boss operating rubric** (one structural principle, not exemplars): resolve parameters you can look up (connected account, default repo, dates) rather than asking; attempt the nearest real tool before declaring inability; if you genuinely cannot, say what you can't do and what you *can*; **and when a live Sheet/Doc already answers the ask, that is the deliverable — stop there, don't chase a binary export** (in `run_m78xa3ei2w4n` the boss had already built the PR Sheet, then poisoned itself reaching for a redundant PDF). **(5) Minimal grounding (F2):** the boss preamble carries the user's connected integration **identities** (e.g. GitHub login `99Yash`, primary repo) so it resolves instead of asking. **(6) The result-honesty primitive:** a tool must surface silent-failure classes at the *result* layer — never a confident zero for a structural reason. For GitHub this is now **defense-in-depth behind ADR-0068's validator** (which already rejects invented qualifier *keys* pre-network): it catches the residue the key-whitelist structurally cannot — a *valid* qualifier with a value GitHub can't satisfy (the validator checks keys, not values), or an `incomplete_results` truncation. Its load-bearing home is ADR-0074's general tier, where calls can't be pre-validated at all.

**Why.** Three prod/dev failures, one root cause: a tool surface that lies, and a boss that gives up on the lie. (i) #222: asked for total LOC across PRs, the boss correctly degraded — search genuinely can't return diff stats — but it *silently dropped* the requirement instead of stating it. (ii) The issue-summary turn (dev `c31d57c2`, **zero tool calls**): the boss's own reasoning shows it knew its tools, found the workaround (*"exploiting any relationship between issues and pull requests"* — correct, `/search/issues` returns issues), realized the request was self-referential (*"my identity as 'Alfred'… self-referential"*), then **abandoned every path** — web search, the PR-search route, its own self-knowledge — and asked the user for the repo. The tool said "pull requests only" though it always could search issues; the boss didn't know it was GitHub-connected as a specific account. (iii) The class also covers result-honesty: a search that *succeeds* with `total_count:0` for the wrong reason. The original `merged-by:@me` confident-zero (ADR-0068's motivating bug) is **already closed** by 0068's qualifier validator — it rejects an invented *key* before the network call — so the live residue is narrower: a *valid* qualifier key with a value GitHub can't satisfy (the validator checks keys, not values), plus the un-validatable general tier (ADR-0074). These are the *same class* as the `export_file` misuse — **bad context about what an API is for / whether a call really answered the question** — which is also ADR-0053's boss-blindness. The fix is truth-in-surface + a resolve-or-state rubric, not a bolt-on `get_issue` (rejected by the owner as exemplar/prompt-patching, per the principles-over-exemplars posture).

**The shape.** GitHub search keeps ADR-0068's `superRefine` whitelist (today `GITHUB_PR_SEARCH_QUALIFIERS` in `contracts/src/github-search.ts`, generalized to span issue+PR qualifiers and to conditionalize the always-appended `is:pr` on `type`) and structured time windows. The boss re-trips the `author:`-in-`query` collision *every turn* (it re-derives the query from scratch and never carries the lesson forward), wasting a call. Surface clarity alone is unlikely to fix that — the description is *already* explicit ("never hand-write those qualifiers into `query`") and the validator already rejects, yet it re-trips; the robust lever (proposed in #213) is **sanitize-and-merge** — strip colliding `author:`/`is:`/`state:`/date qualifiers out of freeform `query` and fold them into the structured fields, turning a hard error + wasted retry into silent correctness — rather than reject-and-retry. `get_pull_request`/`get_issue` are plain `fetch` against `GET /repos/{o}/{r}/{pulls|issues}/{n}` with the existing installation-token credential resolver; `riskTier:'low'`. The text-export allowlist and the rubric line live in `@alfred/contracts`/the boss prompt base respectively; F2 identity injection extends the boss preamble's connected-catalog (ADR-0053) with the *binding* (which account/repo), the minimal first brick of the #218 user-model. Behavioral evals (evalite, ADR-0055): the boss searches issues with a structured/whitelisted query, totals LOC via fan-out, and never answers a confident zero on an unrecognized qualifier.

**Maintenance policy (the schema-sourcing question).** Integration tool schemas are **hand-curated Zod sized to the surface the boss uses**, not generated — because the bug-prone layer (query-DSL semantics: GitHub search qualifiers, Gmail `q` operators) is **prose-documented and absent from any machine-readable spec** (OpenAPI/Discovery/GraphQL cover *endpoint shapes*, not DSLs), so there is nothing to generate it from. Drift on the curated set is caught by **cheap live-API evals**, not a codegen pipeline. Breadth ("do anything") is *not* pursued by hand-modeling every endpoint — it comes from the general tier (ADR-0074). The two tiers share **one** correctness primitive: result-honesty (#6) — the half of correctness that scales to calls you cannot pre-validate.

**Alternatives.** (a) **Add a `get_issue` tool and a prompt bullet** — rejected: treats the symptom; the disease is the lying surface + premature give-up. (b) **Reverse ADR-0068 into a raw native-DSL pass-through** (the owner's first instinct) — rejected after re-reading 0068: it re-opens the confident-wrong-answer hole (silent zero on bad qualifiers) for marginal gain, since the whitelist already permits the *entire valid* DSL and only blocks qualifiers GitHub doesn't have. Raw passthrough is the right *long-tail* mechanism, but with result-honesty + risk controls — that's ADR-0074, not a reversal here. (c) **Generate schemas from OpenAPI/Discovery** — rejected now: heavy for ~15 tools, and it can't cover the DSL layer that actually broke; revisit only if the curated set grows large. (d) **Full identity/relationship modeling for F2** — deferred to #218; only the minimal connected-account binding ships here.

**Cross-ref.** Extends ADR-0053 (connected catalog → adds the *binding*) and ADR-0068 (keeps + extends the validated GitHub surface; does **not** reverse it). The honest read-tool half is the tool-level companion to ADR-0070's platform sanitizer. The result-honesty primitive is inherited by ADR-0074's general tier. First brick of the #218 user-model spine. The deferred produce-artifact capability is ADR-0075's epic.
...
**Why.** The owner's instinct ("if it's in the API it should be there; ideally do anything; users will bring their own MCP servers") is correct for the **invocation/coverage/composition axis** — and Code Mode is the strongest known answer there (fixed token cost regardless of endpoint count, composition without per-call LLM round-trips, auto-binding arbitrary servers). But — confirmed from the Code Mode posts — it *"addresses composition and invocation… no parameter validation or schema enforcement beyond what the model infers."* So Code Mode does **not** solve the bug that motivated this batch: GitHub's silent-zero on a bad qualifier is a **correctness-of-a-given-call** property, orthogonal to how the call is invoked; the model writing `q:"merged-by:@me"` *in sandboxed code* still gets a confident zero. The two axes must not be conflated: this batch builds correctness (ADR-0070/0071/0072); the general tier is breadth, and it still needs the result-honesty primitive on top.

**Constraints to design against (when picked up).** Deployment is **Railway + Cloudflare R2**, runtime is the **Vercel AI SDK** — *not* Cloudflare Workers, so Code Mode's free isolate (V8, no-FS, outbound-off, OAuth-downscoped) is not in hand. Hosting model-authored code that holds the user's Gmail/GitHub/Railway credentials is a **prompt-injection exfiltration surface**; an equivalent sandbox must be built or borrowed (isolated-vm / a Deno subprocess / a microVM, or spinning up CF Durable-Object/facet isolates from Railway) and whether the Vercel AI SDK can cleanly host a single `execute()` tool is open. The (b) passthrough also needs a cross-integration risk model (read-only scoping, cost attribution, auditability) — the same weigh-in deferred from the GitHub grill.
...
**Scope and residual risk.** v1 fixes the **authoring/create** path only. `update_artifact` full-replacement (cross-turn edits) keeps the 500K cap and remains a single re-emit that can still time out on a large document — mitigated only by Gap 1 retry + per-turn effort sizing (#478); mid-document *surgical* edits stay deferred. Two similar append verbs (`_page` vs `_section`) could invite tool-selection confusion — mitigated because the `wrong_kind` guard makes a mis-pick a self-correcting reissue. Because sections concatenate verbatim and the sidebar re-renders the *accumulated* markdown after each poke, the model must split at block boundaries and keep each section self-contained (closed code fences), or an intermediate render hits the ADR-0075 bare-fence `MarkdownRenderer` crash class — a description constraint. The cap was tuned 12K → 15K after the live re-probe (below).

**Alternatives.** (a) **Steer the `pages` descriptions toward incremental authoring** — rejected: `pages` already self-chunks; this perturbs working behavior and fixes nothing (the "A-trap"). (b) **Empty `create` (no markdown), all content through `append`** — an exact `pages` mirror; rejected because a 12K-capped optional `create` is itself already ≤~60s-bounded, so forcing an extra round-trip on every short document buys only symmetry. (c) **Rely on the description alone with no cap** — rejected: no hard bound on a single stored write, and no consistency for the non-compliant sub-timeout case. (d) **Mid-stream tool-argument-size abort** (inspect the streamed partial arg, abort + return a "chunk it" signal when it crosses the cap) — the only way to make the cap *truly* forcing against timeouts; deferred as the escalation lever if the live probe shows the model one-shotting past the description. (e) **Server-side decomposition** (the boss emits an outline, the server fans sections out) — framed-future for truly huge documents or if the model proves unreliable at self-chunking. (f) **Effort-capping as the primary lever** — rejected: that trades depth for speed and addresses the single-dense-artifact *thinking* overrun (#478), a different failure mode.

**Cross-ref.** Extends ADR-0075 (artifact epic; `decisions.md:4040`) and structurally supersedes Gap 1 / #511 for documents (retry catches the residual). Files: `packages/contracts/src/tools.ts` (`INTEGRATION_ACTIONS.system` + `TOOL_LABELS`), `packages/contracts/src/tool-schemas.ts` (`createArtifactInput`, new `appendArtifactSectionInput`, `TOOL_INPUT_SCHEMAS`), `packages/contracts/src/artifacts.ts` (stored 500K unchanged), `packages/api/src/modules/tools/system.ts` (tool decl + description rewrite), `packages/api/src/modules/artifacts/write.ts` (`appendArtifactSection`, template = `appendArtifactPage` `:121-191`), `packages/api/src/modules/agent/workflows/chat-turn.ts` (`ARTIFACT_MUTATION_TOOL_NAMES` — section appends join the ordered mutation lane; the row lock stops lost writes but not reordering). Plan: `docs/plans/incremental-document-authoring-v1.md`. Lesson: `.lessons/incremental-authoring-is-document-gap-not-pages-steering.md`. **Deferred:** cross-turn surgical section edits; mid-stream arg-abort; server-side decomposition.
