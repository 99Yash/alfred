...
**Scope and residual risk.** v1 fixes the **authoring/create** path only. `update_artifact` full-replacement (cross-turn edits) keeps the 500K cap and remains a single re-emit that can still time out on a large document — mitigated only by Gap 1 retry + per-turn effort sizing (#478); mid-document *surgical* edits stay deferred. Two similar append verbs (`_page` vs `_section`) could invite tool-selection confusion — mitigated because the `wrong_kind` guard makes a mis-pick a self-correcting reissue. Because sections concatenate verbatim and the sidebar re-renders the *accumulated* markdown after each poke, the model must split at block boundaries and keep each section self-contained (closed code fences), or an intermediate render hits the ADR-0075 bare-fence `MarkdownRenderer` crash class — a description constraint. The 12K cap is calibrated from a single measurement (196 chars/s) and is tunable after the live re-probe.
