...
## ADR-0077 — Interactive-chat Auto tier runs on Claude Haiku 4.5; Opus 4.8 reserved for Deep (BUILT — #312)

**Decision.** `getChatModel("standard")` — the **Auto** chat tier, the everyday conversational driver and ~87% of LLM spend — moves from Claude Sonnet 4.6 to **Claude Haiku 4.5** (`claude-haiku-4-5-20251001`). The **Deep** tier stays on **Opus 4.8**: the heavy, expensive model is *reserved* for the hard, multi-step turns (and the fan-out boss-worker harness) where the cost buys real reasoning. Both tiers keep their Gemini 2.5 Pro `withFallback` degrade. Because Haiku 4.5 hard-400s on adaptive thinking *and* the `effort` param (both Sonnet-4.6+/Opus-only), `getChatProviderOptions("standard")` sends an **empty** anthropic provider block — sending either would, via `withFallback`'s switch-on-4xx, silently drop every Auto turn onto Gemini (the #224 class of bug). Deep keeps `thinking:{adaptive}` + `effort:"high"`.

**Why.** A browser-replay adjudication (Sonnet 4.6 vs Gemini 2.5/3.5-flash vs Haiku 4.5, real recorded dev chats replayed in the live app, costs recomputed from Langfuse token counts) found Haiku the only candidate that wins on **both** cost and quality. It is ~3× cheaper per token than Sonnet ($1/$5 vs $3/$15) and the saving actually *lands* — unlike Gemini 3.5-flash, which passed quality but cost *more* in practice (2.6×/5.3× token bloat → 1.56× full-price regression) and needed a tool-name shim. Same-provider means Haiku keeps the #223 prompt cache (same tokenizer, ~90% warm-input discount) and the dotted-tool-name handling, so there is no orchestration tax. On tool-heavy chat Haiku matched Sonnet's trajectory; its one observed weakness — under-acting on implicit agentic lookups (asking the user for a sender address instead of searching Gmail) — was an *underspecified prompt* Sonnet's stronger inference had been masking, not a hard capability ceiling: the search-before-ask prompt hardening (#312) closed it, verified live (the boss now runs `gmail.search` → `system.remember` → `system.resolve_todo` itself), and a `sender-suppression-grounding` eval pins it against regression. Reserving Opus for Deep keeps the capability ceiling available exactly where users opt into it, while the default path gets faster and ~3× cheaper.

**Alternatives.** (a) **Keep Sonnet on Auto** — rejected: pays a 3× premium for quality the adjudication showed Haiku matches on real chat traffic; the cost lever on the 87%-of-spend line is too large to leave. (b) **Gemini 2.5-flash or 3.5-flash on Auto** — rejected: 2.5-flash failed quality (mis-orchestrated, punted), 3.5-flash was a *cost regression* despite a lower sticker price, and both need the `.`→`__` tool-name shim and forfeit the prompt cache (`project_gemini_chat_boss_adjudication`). (c) **A capability map driving the thinking/effort block off the resolved model id** — *deferred to #313*: the right long-term shape (so a future tier remap can't reintroduce an unsupported param), but for a fixed two-tier mapping a `tier === "deep"` branch is correct and minimal; the TODO is in the code. (d) **Sonnet as a middle tier between Haiku and Opus** — rejected for now: the chat UI exposes only Auto/Deep, so a third tier has no surface; revisit with the #249 model-router. (e) **Block on a wider eval corpus before flipping** — declined: the adjudication is n=2 chats, but the search-before-ask eval + the non-gating nightly grounding-evals run (#312) give ongoing regression coverage, and the fallback + per-thread provider stickiness bound the blast radius of a bad turn; widening the corpus continues in the eval lane rather than gating the swap.

**Cross-ref.** Consumes the search-before-ask prompt hardening + `sender-suppression-grounding` eval + nightly `grounding-evals` workflow, all shipped in #312, as the safety net that makes the swap defensible. The `getChatProviderOptions` tier→capability branch is the seam ADR-0053/#313's per-model capability map will absorb. Ties to the #249 model-router (which may later make Auto self-route across tiers rather than pin to Haiku). Supersedes the informal "chat runs on Sonnet, escalates to Opus" mapping the `getChatModel` comment carried as "ADR pending". Cost context: `project_llm_cost_shape` (Anthropic = 96% of spend, Sonnet = 87%).
...
## ADR-0083 — Pristine artifacts via a design-system + client-verified floor: freehand-HTML flagship on the active chat tier, structured→Slides as the editable escape hatch (DESIGNED — grilled 2026-07-01, see docs/plans/pristine-artifacts-v1.md)

**Decision.** Agent-built decks/docs optimize a **pristine read-only** deliverable as the flagship — **freehand HTML authored against an Alfred design system**, not structured `{title,bullets,layout}` slots — with a thin **structured→Google Slides escape hatch** for "edit it in Slides." Quality rests on a **scaffold + a client-verified floor**, not on a stronger model: artifacts author on **whatever chat tier is active** (Haiku 4.5 on Auto, ADR-0077) — no model gating, no dedicated sub-agent, no forced tier bump. The scaffold is `@alfred/artifacts-design`, a **pure web-safe package** (`tokens.ts` as the single source of design truth → a generated shell + a design-system prompt + Alfred-original theme/exemplar HTML; zero Node deps, so `apps/web` and `@alfred/api` both import it and `check:web-boundaries` holds by construction). The design system is **delivered as the `create_artifact` result** (progressive disclosure, like `system.load_integration`) — not baked into the tool description or system prompt — so its ~1000-line cost is paid only on turns that author an artifact (never on the 87%-of-spend conversational path) and sits in the transcript prompt-cached across the N `append_artifact_page` calls. The **shell is injected at render time** (extending the existing `PAGE_RESET` prepend in `artifact-page-frame.tsx`): token-generated base CSS + self-hosted subset fonts + inline SVG icons, **no CDN / no full Tailwind**, one shell source wrapping both the in-app render and the export. Pages therefore stop being self-contained and become **shell-dependent** (amends ADR-0075) — the agent writes only page-specific `<style>` + body. The **verified floor is client-measured**: the render iframe becomes scriptable (`sandbox="allow-scripts"`, deliberately *not* `allow-same-origin`) only after model HTML is sanitized down to inert markup — no `<script>`, event-handler attributes, `javascript:` URLs, active SVG/scriptable surfaces, external network loads, or model-authored `postMessage`; only Alfred's injected harness runs, and fit reports carry a per-render nonce so the host ignores spoofed messages. The harness measures `scrollHeight/Width` vs the logical page box and `postMessage`s overflow back; on overflow the server enqueues an **invisible bounded repair pass** (a background run, not a visible chat turn) that `update_artifact`s the offending page, **≤2 retries**, with the artifact held **`generating` until fit is confirmed**. Every exemplar also constrains overflow **by construction** (bounded content box + content caps + min font size) as the backstop when no live browser reports. Multi-slide/ambiguous decks get a **prompt-only soft-outline** in the boss's conversational reply (no formal HIL); authoring is **sequential** (`append_artifact_page` per page, no parallel fan-out) and reuses the existing `pages` kind (**no new `deck` kind**). Read-only **export (Phase 3, deferred) is one engine — in-house headless Chromium** (Playwright/Puppeteer) rendering the shared shell → PDF for both `pages` and `document` (markdown → design-system HTML → Chromium); **Typst is dropped** from v1. **Vision-repair defers** to that same Chromium (flag-gated) because a sandboxed iframe can't be screenshotted client-side.

**Why.** The blank-deck bug (#, 2026-07-01) was Haiku producing two empty Google Slides decks then falling back to an *ad-hoc-CSS* artifact — proving the gap is **pristine quality of a freehand render**, not the ability to emit HTML. Dimension's proven recipe makes freehand output pristine via a heavy prompt cage + exemplar library + a bounded-box shell, all portable (it's a prompt + assets, not infra); Alfred already ships the same freehand-HTML school (ADR-0075), so borrowing the recipe is lower-friction than pivoting to structured slots (which cap quality at "generic"). The **follow-the-picker** bet is the crux: the entire reason to build the scaffold is that it lifts a small model, and Alfred deliberately moved the 87%-of-spend path to Haiku *for cost* — so escalating artifact authoring to Sonnet/Opus would forfeit that saving for a benefit the scaffold + floor are designed to make unnecessary; a sub-agent would also need the parent's `threadId`/`messageId` (the artifact tools refuse without it) plus ADR-0073 join latency. Making the floor **client-measured** removes a latent ordering bug in the first draft (a "deterministic headless fit-check" secretly needed the very Chromium the plan deferred) and lets the #1 failure mode — overflow — be guarded in v1 **with no new infra**: measurement runs in the user's browser, repair is an ordinary `update_artifact` on the existing agent worker. Committing to **in-house Chromium** for export (private artifact content must not go to a rendering SaaS or an agentic-browser cloud) then knocks out Typst's only real justification ("Chromium-free"); Typst would additionally need its own `tokens→Typst` binding — a *third* design target that fights the one-shell-source principle — so one engine is both simpler and more drift-proof. **Progressive-disclosure delivery** of the design system is the cost-correct and quality-correct home: it avoids taxing every conversational turn and puts the scaffold freshly in front of Haiku at the exact moment of authoring. Alfred's workers are **co-hosted with the API in one process** today, so on-demand Chromium adds **no required new service** — only a transient memory spike during a rare export, with a dedicated export worker as a build-time contingency if that spike ever threatens the co-hosted workers.

**The shape (for the build).** Phase 1 (no infra): `@alfred/artifacts-design` (tokens → shell + prompt + one house theme, ~3–6 by-construction fit-safe exemplars); return the design system from `create_artifact`; retrofit the renderer to wrap the shell. Phase 2 (no infra): scriptable measuring iframe + postMessage channel + the invisible bounded repair pass + the `generating`-until-fit lifecycle. Phase 3: in-house on-demand Chromium for all PDF export + flag-gated vision-repair on the same browser. Phase 4: structured→Google Slides (folds in `artifact-native-mirror-v1.md` Option B) + the guaranteed-artifact/honest-fallback prompt rule. Phase 5: theme/archetype breadth (toward Dimension's 8×17), native sheets/docs mirrors, and Typst only if long-form print quality becomes a felt gap. Themes are **Alfred-original** (built on Dimension's principles, not copied — clean IP + an honest "studied their approach, built my own system" story).

**Alternatives.** (a) **Structured-slot authoring for the flagship** (the original `artifact-native-mirror` Option B) — rejected: caps pristine quality at generic; kept only as the *editable* escape-hatch lane, whose job is editability, not polish. (b) **A shared quality-preserving HTML↔structure IR feeding both lanes** ("author once, render many") — rejected for v1: nobody (not even Dimension) has built an HTML-quality-preserving transpiler; forcing one sacrifices the pristine bar. It stays a later epic; re-skinning in v1 = re-author freehand from the soft-outline. (c) **Escalate artifact authoring to Sonnet/Opus or a dedicated sub-agent** — rejected: forfeits the Haiku cost win the scaffold is meant to make unnecessary, and adds `threadId` plumbing + join latency; revisit only if the Phase-1 render on Haiku visibly can't clear the bar even with the scaffold. (d) **Server-headless deterministic fit-check in v1** — rejected: needs the Chromium deferred to Phase 3; the client measures for free. (e) **Vision-repair in v1** — rejected: needs a server screenshot (client can't raster a sandboxed iframe) + per-page vision cost + a new service, for aesthetics the exemplars + client floor already floor; it's a flag-gated fast-follow. (f) **Typst for doc PDFs** — rejected once in-house Chromium is committed: a second engine + second `tokens→Typst` binding that fights one-shell-source; documented future option. (g) **CDN shell (Tailwind/Google Fonts/FA) à la Dimension** — rejected: violates self-containment, can't fit the 200KB/page cap, leaks IP to Google Fonts; render-time injection of a token-generated shell + self-hosted fonts is offline-safe and privacy-aligned. (h) **Design system in the `create_artifact` tool description** (Dimension's home) — rejected: taxes every chat turn's cached prefix; the result-delivery pays only when authoring. (i) **Managed HTML→PDF SaaS or browser-use cloud for export** — rejected on privacy: ships the user's private artifact to a third party; browser-use is also the wrong abstraction (agentic web automation, Python) for rendering our own known HTML — plain Playwright/Puppeteer is the primitive. (j) **Formal HIL outline gate** — rejected: artifacts are `no_risk` and edit-in-chat already exists; a prompt-only soft-outline gets the cost-avoidance + control without the approval machinery. (k) **A persistent Browserless sidecar** — rejected: idle Chromium 24/7 for rare single-user exports; on-demand in-process is cheaper, with a dedicated worker only if memory demands.

**Cross-ref.** **Amends ADR-0075** (the self-contained-page invariant becomes shell-dependent; the render iframe gains `allow-scripts`). **Reframes `docs/plans/artifact-native-mirror-v1.md`** as the escape-hatch spec (its Option B demoted from primary to the editable lane) and **supersedes** the "populate-on-create" line in ADR-0043's write-surface plan. Runs on the ADR-0077 chat tiers (Haiku Auto / Opus Deep) with no artifact-specific override. Reuses ADR-0073's sub-agent/background-run machinery for the invisible repair pass and the Replicache entity recipe for the synced `artifacts` row. Honors the no-em-dashes copy voice (`feedback_copy_voice`) — which Dimension's own prompt independently bans. Recon + benchmark evidence: memory `project_artifact_document_engine`. **Parked (out of scope):** vision-repair (Phase 3, flag-gated); Typst; a shared HTML↔structure IR; native Google Doc mirror; the `spreadsheet` kind; theme breadth beyond v1's 1–2.
