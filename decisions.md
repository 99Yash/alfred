...
**Decision.** A failed chat turn's `error_kind` is gated by **structure first, then narrowed by an intentional signal** — never by an over-broad substring net — and a message is only finalized `failed` when the fault is genuinely **terminal**. Three changes, in priority order: **(1) A presence gate is the load-bearing fix.** `error_kind:'attachment'` is returned only when the turn actually carried a `ready` attachment (derivable from `loadReadyAttachments`, threaded into the failure path as `turnHadAttachment`); with no attachment it is structurally impossible, so a `drive.export_file`/tool/persistence failure on a no-attachment turn classifies as `generic` ("Something interrupted this reply") — no new kind. This alone resolves the reported #269. **(2) Narrow the classifier to the genuine attachment-failure signal.** The legitimate `attachment` case is the **model provider rejecting a hydrated image** at the generation call (`"unable to process input image"` and the `invalid`/`unsupported image` family), recurring on transcript replay (`.lessons/chat-vision-transcript-replay-poison.md`). `classifyChatFailure` keeps only that narrow signal set — **gated on `turnHadAttachment`** — and the over-broad `mentionsAttachment` net (`attachment|file|image|media|mime`, `chat-turn.ts:1319-1340`) is **deleted**, because that net is exactly what mis-bucketed the export failure. Attachment *hydration* keeps its existing graceful behavior — a bad image is swallowed per-image into a placeholder text part and the turn continues (`chat-turn.ts:444-463`); it is **not** promoted to a turn-failing throw (that would regress graceful degradation, and hydration is not where the user-visible attachment failure surfaces today). **(3) Terminal-aware finalization**: `finalizeFailedMessage` marks the `chat_messages` row `failed` only on a genuinely terminal fault, not on an error that the executor will retry/reclaim — so the message status and `agent_runs.status` agree.
...
**The shape.** `turnHadAttachment` is computed from `loadReadyAttachments` for the current user turn and threaded into the failure path; `classifyChatFailure(err, { turnHadAttachment })` returns `attachment` only when the flag is set **and** the error matches the narrow provider-image-reject signal set, else `generic`; the broad `mentionsAttachment` net is removed. Hydration's per-image swallow-to-placeholder (`chat-turn.ts:444-463`, today raising `UnsupportedStoredImageError` caught locally) is left intact — it is *not* the failure surface. An optional tightening is to wrap the recognized provider reject in a tagged error at the catch site for cleanliness, but the recognition stays signal-based — a provider error carries no structured tag, so some intentional matching there is unavoidable; the *gate*, not a tag, is what makes it correct. `finalizeFailedMessage` is called only from the terminal-fault path (the #267 backstop guarantees poison/commit failures *reach* terminal rather than looping, which is what makes "finalize only when terminal" coherent). Client `FAILURE_PRESENTATION` is unchanged — once the kind is right, a tool failure renders the honest `generic` copy instead of the nonsensical attachment chip.
