...
| Memory store | **Postgres substrate over a graph DB**; build the memory *intelligence*, don't buy the *store*. ADR-0067 makes the user model a **multi-source observation log as system of record**, with stable `entities`/`entity_identities` as the foreign-key surface and `entity_profiles`/`entity_relations`/`co_occurrence`/significance/`user_facts` as replayable projections. Graph as a **model**, not a graph **engine** (Neo4j/FalkorDB); adopting Zep/Mem0 would move truth off Postgres and break Replicache/reversibility. Borrow external patterns, not their store (ADR-0058/0067) |
...
**Sequencing.** ADR + plan first (this), then incremental, Gmail-first, each phase shadow-built + validated: **P0** schema + contracts + projection run tracking + Replicache projection-sync state; lock stable `entities`/`entity_identities` vs versioned `entity_profiles`/`entity_relations`/`co_occurrence`/`user_facts`, plus stable sync keys/content-versioning for Replicache-visible active views, before schema generation · **P1** Gmail end-to-end + validate vs today, including the `engineering@oliv.ai` group/unscored fix · **P2** GitHub (commit-email bridges + wire stable-ID object_relations) · **P3** Calendar + conditional Google Workspace Directory (new ingestion, highest first-party people signals; verify `https://www.googleapis.com/auth/directory.readonly` is grantable and the `oliv.ai` admin-side profile/contact sharing toggle is enabled before adding it to `GOOGLE_FEATURE_SCOPES`) · **P4** fact ontology (subject-binding + purge; migrate all non-proposed user-authoritative facts into user observations; replay-derived status/confidence) · **P5** consumer cutover (triage/briefing/meeting-prep + re-scoped 0066) + Replicache delta validation + drop legacy.

**Parked / open (decide during build).** `FAN_OUT_CUTOFF` (~10–15?), `PROMOTION_THRESHOLD`, the `SOURCE_WEIGHTS` constants (hand-tuned + documented, not learned at single-user scale); deterministic merge survivor/display-primary ordering when hard-bridged stable entities collapse; provisional-entity auto-merge aggressiveness once a bridge appears; whether group rosters (`member_of`) are reliably inferable from list-addressed mail or need Directory/explicit source; final active-view names; Google Workspace Directory scope/admin-sharing verification; third-party enrichment is post-v1, proposed-only + cited, layered on ADR-0057's existing significance-gated enrichment path, not a new dependency; evalite coverage for the projection. Plan: [docs/plans/multi-source-user-model-v1.md](./docs/plans/multi-source-user-model-v1.md).
