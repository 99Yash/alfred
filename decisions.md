...
| [0073](./docs/decisions/ADR-0073-sub-agent-join-via-a-child-completion-wake.md) | Sub-agent join via a child-completion wake signal, not scratch-polling (BUILT + reliability backstops PR #274; finalization invariant deferred — see amendment) |
...
| [0078](./docs/decisions/ADR-0078-per-model-capability-is-code-resident.md) | Per-model capability is code-resident (effort *vocabularies*, not booleans) on a per-provider dispatch profile; models.dev is the audit oracle, not a runtime dependency (BUILT — #313) |
