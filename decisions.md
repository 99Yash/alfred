...
- **Storage (Postgres; ADR-0058 "graph as a *model*, not an *engine*").**
  ...
  - **`integration_object_keys`** — sidecar `(user_id, provider, key_kind, key_value) → object_id`. `head_sha`→PR, `branch`→PR, `run_id`→PR, `task_id`→task all resolve uniformly. The `head_sha`→PR index is the exact thing the prod recon proved necessary (the CI email carries a head-sha and *no* PR number).
