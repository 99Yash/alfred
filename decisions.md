...
2. **User-authored slug resolution via a single sentinel `Workflow<S>`.** `requireWorkflow(slug)` returns the registry hit when one exists; otherwise it returns the shared `userAuthoredBriefWorkflow` sentinel. `createRun` does an existence check on `workflows (userId, slug)` when the registry misses; a missing row OR `is_builtin=true` throws (covers deleted-builtin deploy bugs); `is_builtin=false` routes through the sentinel. `createRun` must still insert the requested slug into `agent_runs.workflow_slug`, not the sentinel slug, so history joins back to the user-authored row.
...
// registry.ts: `requireWorkflow` falls back here on miss.
// createRun: existence check on (userId, slug) + is_builtin=false guard before
//            serving the sentinel — typos and deleted builtins fail loud.
```
