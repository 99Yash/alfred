...
- Reach for an existing helper before writing a new `format*`/`parse*`/`is*`/`to*`/`get*` function or adding to a route `helpers.ts`. Don't create new grab-bag `utils.ts`/`helpers.ts` for anything generic. Three things find the helper for you, so this list stays short:
  ...
  | any calendar-day / zone / offset reading       | `settings.resolveTimezone` for the zone, then the timezone module — `inZone(tz).day()` / `.hour()` / `.dayBounds()` / `.startOf(key)` / `.clock()` for anything needing one, and `addDays` / `weekdayIndex` / `formatDay` on the key for anything that doesn't. Zones are `IanaTimezone`, days are `LocalDateKey` | `@alfred/assistant/time` (math), `@alfred/assistant/settings` (`resolveTimezone`) |
  ...
  | enforce prose voice                            | `sanitizeVoice`                                                                                                                                                                                                                                                                                                   | `@alfred/ai/voice`                                                                |

- Do not add tests for a feature. This includes value-equality tests, type fixtures with `@ts-expect-error` probes, and schema round-trip tests. Make the compiler, a runtime parse at the owning boundary, or a `pnpm check` gate carry the proof instead. When a behavior cannot be proved that way, state the residual risk in the PR body.
