...
| --- | --- | --- |
| `intake` | Artifact → one item file per finding, `state.json` queue. Reconciles against `git log` to drop findings already landed. **The only phase that reads the whole artifact.** | every item to `design` or `cover` |
...
| `implement` | Worktree, branch, TDD at the agreed seam, `pnpm check`, commit, push, draft PR. | `review` |
